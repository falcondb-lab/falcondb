...
### <a id="not-supported"></a>不支持 (v1.2)

以下功能**明确不在** v1.2 范围内。尝试使用会返回包含对应 SQLSTATE 代码的 `ErrorResponse`。
...
> **Standard 版定位**：Standard 版以 RocksDB 为默认存储引擎——磁盘优先、分布式、完全兼容 PostgreSQL，可无缝迁移现有 PG 应用。通过 `--features standard`（或 `--features edition_standard`）构建。
...
FalconDB 内置**自治运维（AIOps）引擎**，完全在进程内运行，无需外部依赖。自动监控查询行为、检测异常并提供可操作建议，全部通过 SQL 访问。

### 四大子系统

#### 1. 慢查询检测器（SlowQueryDetector）

基于 **EMA p95 基线**的自适应阈值，而非固定超时。

- 初始阈值下限：100 ms
- 自适应阈值：`max(100ms, ema_p95 × 2.0)`，随负载自动收紧
- 预热期（前 100 次查询）使用绝对阈值下限
- 保留最近 200 条慢查询记录（滚动窗口）
- 每条记录含截断原始 SQL（前 200 字符）和主表名

#### 2. 异常检测器（AnomalyDetector）

基于 **Welford 在线方差**的 3σ 规则，监控两个独立信号：
...
- 需要 ≥ 30 个每秒观测样本才激活（冷启动无误报）
- TPS 突增或延迟异常触发 `WARNING`
- 延迟超过均值 5 倍时触发 `CRITICAL`
- **连续 ≥ 5 次慢查询**触发 `WARNING`（source: `AnomalyDetector/SlowQuery`）
- 保留最近 500 条告警（滚动窗口）

#### 3. 索引推荐器（IndexAdvisor）

按表名追踪**全表扫描事件**：

- 表累计 ≥ 10 次全扫描且平均扫描时间 ≥ 50 ms 时生成推荐
- 识别 WHERE 子句中使用频率最高的 Top-3 过滤列
- 生成可直接执行的 `CREATE INDEX` 语句
- 按影响分（`全扫描次数 × 平均耗时`）降序排列

#### 4. 工作负载分析器（WorkloadProfiler）

按查询**指纹聚合**执行统计（规范化 SQL，取前 120 字符）：

- 跟踪每个指纹的：调用次数、总/均/最大延迟、**p95/p99**（reservoir sampling，100 样本）、错误次数
- 最多支持 1000 个不同查询指纹
- 支持按总耗时或调用次数排名 Top-N 查询
...
#### `SHOW AIOPS STATS` 输出字段

| 指标 | 说明 |
|------|------|
| `slow_query_samples` | 慢查询检测器处理的总查询数 |
| `slow_query_threshold_us` | 当前自适应慢查询阈值（µs） |
| `slow_query_ema_baseline_us` | 用于阈值计算的 EMA p95 基线 |
| `slow_query_logged` | 滚动缓冲区中的慢查询数量 |
| `anomaly_tps_mean` | TPS 运行均值 |
| `anomaly_tps_stddev` | TPS 运行标准差 |
| `anomaly_latency_mean_us` | 延迟运行均值（µs） |
| `anomaly_latency_stddev_us` | 延迟运行标准差 |
| `anomaly_alerts_total` | 累计触发的告警次数 |
| `index_advisor_tables_tracked` | 有全扫描历史的表数量 |
| `index_advisor_advice_count` | 当前有效索引推荐数 |
| `workload_fingerprints` | 已追踪的不同查询形状数 |

#### `SHOW AIOPS ALERTS` 输出字段

| 字段 | 说明 |
|------|------|
| `id` | 单调递增的告警 ID |
| `ts_unix_ms` | 告警时间戳（Unix 毫秒） |
| `severity` | `WARNING` 或 `CRITICAL` |
| `source` | `AnomalyDetector/TPS` 或 `AnomalyDetector/Latency` |
| `message` | 包含观测值与基线的可读描述 |

#### `SHOW AIOPS INDEX ADVICE` 输出字段

| 字段 | 说明 |
|------|------|
| `table_name` | 频繁全扫描的表名 |
| `full_scan_count` | 记录的全扫描次数 |
| `avg_scan_duration_us` | 平均全扫描耗时（µs） |
| `column_hints` | WHERE 子句中的高频过滤列 |
| `suggestion` | 可直接执行的 `CREATE INDEX` 语句 |

#### `SHOW AIOPS WORKLOAD` 输出字段

| 字段 | 说明 |
|------|------|
| `fingerprint` | 规范化 SQL 指纹（前 120 字符） |
| `call_count` | 总调用次数 |
| `avg_us` | 平均执行时间（µs） |
| `p95_us` | p95 延迟估算（reservoir sampling） |
| `p99_us` | p99 延迟估算（reservoir sampling） |
| `max_us` | 最大执行时间（µs） |
| `total_us` | 总 CPU 时间（µs） |
| `error_count` | 返回错误的执行次数 |

#### `SHOW AIOPS SLOW QUERIES` 输出字段

| 字段 | 说明 |
|------|------|
| `ts_unix_ms` | 检测到慢查询的时间戳（Unix 毫秒） |
| `duration_us` | 实际执行时间（µs） |
| `threshold_us` | 检测时的自适应阈值（µs） |
| `plan_kind` | 物理计划类型（如 `SeqScan`、`IndexScan`） |
| `table_name` | 主扫描表名 |
| `sql_text` | 截断的原始 SQL（前 200 字符） |

### 数据流

```
每次查询执行（Executor::execute）
        │
        ├─ SlowQueryDetector.record(指纹, 耗时, 计划类型)
        │       └─ 更新 EMA 基线，慢查询写入滚动缓冲区
        │
        ├─ AnomalyDetector.record(耗时)
        │       └─ 按秒聚合 TPS+延迟 → Welford 更新 → 3σ 告警
        │
        ├─ IndexAdvisor.record_full_scan(表名, 耗时, 过滤列)
        │       └─ 仅用于 SeqScan 计划；累加每表统计
        │
        └─ WorkloadProfiler.record(SQL, 耗时, 是否出错)
                └─ 规范化指纹，更新每指纹统计
```

### 集成方式

AIOps 引擎使用**全局单例**（`global_aiops()`），所有 Executor 实例共享同一实例，无需配置，统计数据自动跨会话累积。
...
所有里程碑至 v1.2 已发布。当前测试数: **4,200+**，跨 18 个 crate (417 个 `.rs` 文件)。
...
| **v1.1** ✅ | USTM 分层内存, 融合流式聚合, 接近 PG 查询对等 |
| **v1.2** ✅ | Standard 版（RocksDB 默认引擎）、版本特性标志、PL/pgSQL 触发器、ColumnStore GC & truncate、`CREATE TYPE ... AS ENUM`、AI 优化器（在线 SGD, `SHOW AI STATS`）、RocksDB GC flush + checkpoint 副本同步 |
...
| 文档 | 说明 |
|------|------|
| [ARCHITECTURE.md](ARCHITECTURE.md) | 系统架构、crate 结构、数据流（英文） |
...
| [CHANGELOG.md](CHANGELOG.md) | 语义化版本变更日志 (v0.1–v1.2) |
...
## 企业版功能（对标 PostgreSQL Enterprise）

FalconDB 在使用 RocksDB 作为默认存储引擎时，实现了与 PostgreSQL Enterprise 等价的核心能力。所有功能均通过标准 SQL 访问，无需外部服务。
...
-- 列：table_name, policy_name, command, permissive, using_expr, check_expr, rls_enabled
...
```sql
-- 创建逻辑复制槽（兼容 pg_logical / pgoutput）
CREATE REPLICATION SLOT my_slot LOGICAL pgoutput;
-- 返回：slot_name, plugin, confirmed_flush_lsn

-- 查看所有复制槽
SHOW REPLICATION SLOTS;
-- 列：slot_name, plugin, confirmed_flush_lsn, restart_lsn, active, created_at_unix_ms
...
执行器内置进程内环形缓冲区（容量 4096），记录每次 DML 操作的审计事件。
...
-- 按表名过滤（匹配 SQL 中包含该表名的事件）
SHOW AUDIT LOG FOR TABLE orders LIMIT 20;
-- 列：event_id, timestamp_ms, event_type, role_name, detail, sql, success
```

### RocksDB 企业级存储引擎

编译时指定 `--features rocksdb` 启用 `rocksdb_engine.rs` 后端，提供以下能力：

| 功能 | 实现 |
|------|------|
| **列族隔离** | 系统 CF / 用户数据 CF / 二级索引 CF / TTL CF |
| **分层压缩** | L0–L2 使用 LZ4（热数据），L3+ 使用 Zstd（冷数据） |
| **TTL / 行过期** | RocksDB `CompactionFilter` + key 内编码过期时间戳 |
| **二级索引** | 独立索引 CF，支持仅索引扫描 |
| **TDE 钩子** | `encryption.rs` AES-256-GCM DEK 接入 RocksDB Env |
| **MVCC** | 每 PK 分片锁，原子提交/回滚批次，版本链编码 |
...
-- 查看后台 schema 变更进度
SHOW DDL STATUS;
-- 列：id, operation, phase, table_id, rows_processed, rows_total, elapsed_ms, error
```

阶段流转：`pending → running → backfilling → completed`（纯元数据变更直接跳到 `completed`）。
