...
| 功能 | 错误码 | 错误信息 |
|------|--------|----------|
| 存储过程 / PL/pgSQL | `0A000` | `stored procedures are not supported` |
| 触发器 | `0A000` | `triggers are not supported` |
| 物化视图 | `0A000` | `materialized views are not supported` |
| 外部数据包装器 (FDW) | `0A000` | `foreign data wrappers are not supported` |
| HTAP / ColumnStore 分析 | — | ColumnStore 存储 + 向量化 AGG 下推已实现；完整分析管道开发中 |
| 自动再均衡 | — | 可通过 `[rebalance]` 配置段启用 |
| 自定义类型 (JSONB 之外) | `0A000` | `custom types are not supported` |
...
| `falcon_planner` | 计划生成, 基于代价的优化器, 路由提示, 分布式包装 |
...
所有里程碑至 v1.2 已发布。当前测试数: **4,350+**，跨 18 个 crate (432 个 `.rs` 文件, ~246K 行 Rust 代码)。
...
cargo test -p falcon_executor  # 300+ 测试
