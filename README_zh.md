...
| 功能 | 模块状态 | 备注 |
|------|:--------:|------|
| Raft 一致性复制 | STUB | `falcon_raft` 为单节点空实现，不在生产路径上，无目标里程碑 |
...
| `falcon_raft` | 共识桩实现（不在生产路径 — 单节点空操作） |
...
| **v1.0 Phase 3** ✅ | 企业版: RLS、TDE、分区、PITR、CDC | 2,056 测试 |
...
FalconDB 支持两种生产持久化策略: `local-fsync`（默认，RPO > 0 可能）和
`sync-replica`（主节点等待副本 WAL ACK，RPO ≈ 0）。基于 Quorum 的策略在代码中已定义但
需要 Raft（未实现）。详见 [docs/rpo_rto.md](docs/rpo_rto.md)。
...
cargo test -p falcon_raft             # 12 测试 (共识桩实现 — 不在生产路径)
