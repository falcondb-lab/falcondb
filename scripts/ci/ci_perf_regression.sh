...
PERF_BASELINE_PATH="${PERF_BASELINE_PATH:-benchmarks/baselines/perf_baseline.json}"
PERF_SKIP_GATE="${PERF_SKIP_GATE:-0}"

CURRENT_PATH="benchmarks/baselines/perf_current.json"
...
if [ "${REGRESSION_FOUND}" -ne 0 ]; then
    ...
    echo "  cp benchmarks/baselines/perf_current.json benchmarks/baselines/perf_baseline.json"
    echo "  git add benchmarks/baselines/perf_baseline.json && git commit -m 'perf: update baseline'"
    echo "============================================================"
    exit 1
fi
