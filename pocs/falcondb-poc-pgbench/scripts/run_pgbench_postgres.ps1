...
<#
.SYNOPSIS
    FalconDB pgbench PoC — Run pgbench Against PostgreSQL (Windows)
#>
...
Banner "PostgreSQL pgbench: clients=$Clients, jobs=$Jobs, duration=${Duration}s, mode=$Mode"

# ── Warm-up ───────────────────────────────────────────────────────────────
Info "Warm-up run (15s, not measured)..."
$warmupArgs = @("-h", $PgHost, "-p", $PgPort, "-U", $PgUser, "-c", $Clients, "-j", $Jobs, "-T", "15")
...
for ($run = 1; $run -le 3; $run++) {
    ...
    $runArgs = @("-h", $PgHost, "-p", $PgPort, "-U", $PgUser, "-c", $Clients, "-j", $Jobs, "-T", $Duration, "--progress=10")
...
Ok "All PostgreSQL runs complete. Raw output: $RawDir"
