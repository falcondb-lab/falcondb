...
async fn run_import_copy(client: &DbClient, cmd: &ImportCmd) -> Result<ImportSummary> {
    ...
    loop {
        ...
        let n = std::io::Read::read(&mut reader, &mut tmp).context("IMPORT: file read error")?;
        if n == 0 {
            break;
        }
    ...
    let rows = sink.finish().await.context("IMPORT: COPY finalize error")?;
    let _ = rows;
...
async fn run_import_insert(client: &DbClient, cmd: &ImportCmd) -> Result<ImportSummary> {
    ...
    debug!(
        "IMPORT INSERT: table={}, columns={:?}, batch={}",
