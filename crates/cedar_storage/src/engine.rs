...
use cedar_common::error::StorageError;
use cedar_common::schema::{Catalog, TableSchema};
use cedar_common::config::{NodeRole, WritePathEnforcement};
use cedar_common::types::{TableId, Timestamp, TxnId, TxnType};
...
/// protocol handler can expose them via `SHOW cedar.replication_stats`.
...
impl StorageEngine {
    ...
    pub fn analyze_table(&self, table_name: &str) -> Result<TableStatistics, StorageError> {
        ...
        let rows: Vec<Vec<cedar_common::datum::Datum>> = table
            ...
            match record {
                ...
                WalRecord::CreateView { name, query_sql } => {
                    ...
                    if catalog.find_view(name).is_none() {
                        catalog.add_view(cedar_common::schema::ViewDef {
                            name: name.clone(),
                        ...
                        match op_type {
                            "add_column" => {
                                if let Some(col_val) = op.get("column") {
                                    if let Ok(col) = serde_json::from_value::<cedar_common::schema::ColumnDef>(col_val.clone()) {
                                        let mut catalog = engine.catalog.write();
                                        if let Some(schema) = catalog.find_table_mut(table_name) {
                                            let new_id = cedar_common::types::ColumnId(schema.columns.len() as u32);
                                            let mut new_col = col;
                                            new_col.id = new_id;
...
pub(crate) fn datatype_to_cast_target(dt: &cedar_common::types::DataType) -> String {
