...
use cedar_common::schema::{StorageType, TableSchema};
...
impl StorageEngine {
    ...
    pub fn alter_table_add_column(
        &self,
        table_name: &str,
        col: cedar_common::schema::ColumnDef,
        ...
        if has_default {
            if let Some(table_ref) = self.tables.get(&table_id) {
                let default_val = col.default_value.clone().unwrap_or(cedar_common::datum::Datum::Null);
                ...
                for entry in memtable.data.iter() {
                    ...
                    if let Some(row) = chain.read_latest() {
                        ...
                        while values.len() <= col_idx {
                            values.push(cedar_common::datum::Datum::Null);
                        }
                        values[col_idx] = default_val.clone();
                        chain.replace_latest(cedar_common::datum::OwnedRow::new(values));
                    }
                    batch_count += 1;
    ...
    pub fn alter_table_change_column_type(
        ...
        table_name: &str,
        col_name: &str,
        new_type: cedar_common::types::DataType,
    ) -> Result<u64, StorageError> {
        let mut catalog = self.catalog.write();
        let schema = catalog
            .find_table_mut(table_name)
    ...
    pub fn alter_table_set_default(
        ...
        table_name: &str,
        col_name: &str,
        default_val: cedar_common::datum::Datum,
    ) -> Result<u64, StorageError> {
        let mut catalog = self.catalog.write();
        let schema = catalog
            .find_table_mut(table_name)
