...
impl DiskRowstoreTable {
    ...
    pub fn new_in_memory(schema: TableSchema) -> Result<Self, StorageError> {
        // Use a temp file
        let tmp_dir = std::env::temp_dir().join("cedar_disk_rs");
...
mod tests {
    ...
    use cedar_common::schema::{ColumnDef, StorageType};
    use cedar_common::types::{ColumnId, TableId, Timestamp, TxnId};
        ...
        TableSchema {
            ...
            columns: vec![
                ColumnDef {
                    id: ColumnId(0),
                    name: "id".to_string(),
                    data_type: cedar_common::types::DataType::Int64,
                    nullable: false,
                    is_primary_key: true,
                    default_value: None,
                    is_serial: false,
