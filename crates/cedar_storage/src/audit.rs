...
//! and authentication failures. Exposed via SHOW cedar.audit_log.
//!
//! ## Hot-path design
...
use cedar_common::security::{AuditEvent, AuditEventType};
use cedar_common::tenant::TenantId;
...
impl AuditLog {
    ...
    pub fn record_login(
        &self,
        tenant_id: TenantId,
        role_id: cedar_common::security::RoleId,
        role_name: &str,
        session_id: i32,
...
mod tests {
    use super::*;
    use cedar_common::security::{AuditEventType, RoleId};
    use cedar_common::tenant::{TenantId, SYSTEM_TENANT_ID};

    #[test]
    fn test_audit_log_record_and_snapshot() {
        let log = AuditLog::new();
        log.record_login(SYSTEM_TENANT_ID, RoleId(0), "cedar", 1, Some("127.0.0.1".into()), true);
        log.record_ddl(SYSTEM_TENANT_ID, RoleId(0), "cedar", 1, "CREATE TABLE t(id INT)", true);
    ...
    fn test_audit_log_capacity_eviction() {
        ...
        for i in 0..5 {
            log.record_login(SYSTEM_TENANT_ID, RoleId(0), "cedar", i, None, true);
        }
    ...
    fn test_snapshot_by_type() {
        let log = AuditLog::new();
        log.record_login(SYSTEM_TENANT_ID, RoleId(0), "cedar", 1, None, true);
        log.record_ddl(SYSTEM_TENANT_ID, RoleId(0), "cedar", 1, "CREATE TABLE x(id INT)", true);
    ...
    fn test_auto_assigned_event_id_and_timestamp() {
        let log = AuditLog::new();
        log.record_login(SYSTEM_TENANT_ID, RoleId(0), "cedar", 1, None, true);
        log.record_login(SYSTEM_TENANT_ID, RoleId(0), "cedar", 2, None, true);
