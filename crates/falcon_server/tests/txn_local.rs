...
fn test_global_txn_classification_from_plan() {
    ...
    assert_eq!(hint.planned_txn_type(), falcon_planner::PlannedTxnType::Global);
}
...
fn test_single_table_classification_from_plan() {
    ...
    assert_eq!(hint.planned_txn_type(), falcon_planner::PlannedTxnType::Local);
}
...
fn test_show_falcon_txn_stats() {
    ...
    let result = run_sql(&storage, &txn_mgr, &executor,
        "SHOW falcon_txn_stats", None).unwrap();
...
fn test_show_node_role_via_sql() {
    let (storage, txn_mgr, executor) = setup();
    let result = run_sql(&storage, &txn_mgr, &executor,
        "SHOW falcon.node_role", None).unwrap();
...
fn test_show_wal_stats_via_sql() {
    ...
    let result = run_sql(&storage, &txn_mgr, &executor,
        "SHOW falcon.wal_stats", None).unwrap();
...
fn setup_read_only() -> (Arc<StorageEngine>, Arc<TxnManager>, Arc<Executor>) {
    ...
    let stmts = falcon_sql_frontend::parser::parse_sql(
    ...
    // Insert a row
    let txn = txn_mgr.begin(IsolationLevel::ReadCommitted);
    let stmts2 = falcon_sql_frontend::parser::parse_sql(
...
fn test_read_only_show_commands_allowed() {
    ...
    assert!(r1.is_ok(), "SHOW falcon.node_role should work on read-only executor");
    let r2 = run_sql(&storage, &txn_mgr, &executor, "SHOW falcon.wal_stats", None);
    assert!(r2.is_ok(), "SHOW falcon.wal_stats should work on read-only executor");
    let r3 = run_sql(&storage, &txn_mgr, &executor, "SHOW falcon_txn_stats", None);
    assert!(r3.is_ok(), "SHOW falcon_txn_stats should work on read-only executor");
}

// 鈹€鈹€ SHOW falcon.connections tests 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€

#[test]
fn test_show_connections_returns_metrics() {
    let (storage, txn_mgr, executor) = setup();
    let result = run_sql(&storage, &txn_mgr, &executor, "SHOW falcon.connections", None);
...
fn test_show_connections_on_read_only_executor() {
    let (storage, txn_mgr, executor) = setup_read_only();
    let result = run_sql(&storage, &txn_mgr, &executor, "SHOW falcon.connections", None);
    assert!(result.is_ok(), "SHOW falcon.connections should work on read-only executor");
}

#[test]
fn test_explain_show_connections() {
    let (storage, txn_mgr, executor) = setup();
    let result = run_sql(&storage, &txn_mgr, &executor, "EXPLAIN SHOW falcon.connections", None);
    assert!(result.is_ok(), "EXPLAIN SHOW falcon.connections should succeed");
    match result.unwrap() {
...
fn test_show_connections_default_values() {
    ...
    let (storage, txn_mgr, executor) = setup();
    let result = run_sql(&storage, &txn_mgr, &executor, "SHOW falcon.connections", None).unwrap();
    match result {
        ExecutionResult::Query { rows, .. } => {
