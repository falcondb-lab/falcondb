...
mod txn_manager_tests {
    ...
    use cedar_common::types::{IsolationLevel, ShardId, Timestamp, TxnId};
    ...
    fn test_prepare_rejects_local_txn() {
        ...
        match result.unwrap_err() {
            cedar_common::error::TxnError::InvariantViolation(id, msg) => {
    ...
    fn test_fast_path_unique_violation_returns_constraint_violation() {
        ...
        assert!(result.is_err());
        match result.unwrap_err() {
            cedar_common::error::TxnError::ConstraintViolation(id, msg) => {
    ...
    fn test_try_begin_rejects_under_pressure() {
        ...
        assert_eq!(
            ...
            cedar_storage::memory::PressureState::Pressure
        );

        let result = mgr.try_begin(IsolationLevel::ReadCommitted);
        assert!(result.is_err(), "try_begin should reject under Pressure");
        match result.unwrap_err() {
            cedar_common::error::TxnError::MemoryPressure(_) => {}
    ...
    fn test_try_begin_rejects_under_critical() {
        ...
        assert_eq!(
            storage.pressure_state(),
            cedar_storage::memory::PressureState::Critical
        );

        let result = mgr.try_begin(IsolationLevel::ReadCommitted);
        assert!(result.is_err(), "try_begin should reject under Critical");
        match result.unwrap_err() {
            cedar_common::error::TxnError::MemoryLimitExceeded(_) => {}
    ...
    fn test_admission_recovers_after_dealloc() {
        ...
        assert_eq!(
            storage.pressure_state(),
            cedar_storage::memory::PressureState::Normal
        );
        assert!(
    ...
    fn test_tenant_id_and_priority_on_txn_handle() {
        let (_storage, mgr) = setup();
        let txn = mgr.begin(IsolationLevel::ReadCommitted);
        assert_eq!(txn.tenant_id, cedar_common::tenant::SYSTEM_TENANT_ID);
        assert_eq!(txn.priority, cedar_common::security::TxnPriority::Normal);
    }
}
