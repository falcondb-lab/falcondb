...
///
/// `values` is stored in a `SmallVec<[Datum; 8]>` so that rows with ≤ 8 columns
/// (the typical OLTP table) never touch the heap allocator for the spine Vec.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OwnedRow {
    #[serde(with = "smallvec_serde")]
    pub values: SmallVec<[Datum; 8]>,
}

impl OwnedRow {
    /// Create from any collection that converts into `SmallVec<[Datum; 8]>`.
    /// Accepts both `Vec<Datum>` (existing call-sites unchanged) and
    /// `SmallVec<[Datum; 8]>` (hot insert path — zero-copy).
    #[inline]
    pub fn new(values: impl Into<SmallVec<[Datum; 8]>>) -> Self {
        Self { values: values.into() }
    }

    /// Alias kept for clarity on the fast-path insert (same as `new`).
    #[inline]
    pub fn from_smallvec(values: SmallVec<[Datum; 8]>) -> Self {
        Self { values }
    }
    ...
    /// `const`-compatible empty row for use in `static` initialisers.
    /// Returns an owned value; callers that need `&OwnedRow` can use
    /// `&OwnedRow::EMPTY`.
    pub const EMPTY: OwnedRow = OwnedRow { values: SmallVec::new_const() };
...
/// Serde helper: serialize `SmallVec<[Datum; 8]>` as a plain `Vec<Datum>`.
/// This keeps the WAL binary format and JSON output identical to the old
/// `Vec<Datum>` representation — no on-disk format break.
pub(crate) mod smallvec_serde {
    use super::{Datum, SmallVec};
    use serde::{Deserialize, Deserializer, Serialize, Serializer};

    pub fn serialize<S>(v: &SmallVec<[Datum; 8]>, s: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
    ...
    pub fn deserialize<'de, D>(d: D) -> Result<SmallVec<[Datum; 8]>, D::Error>
    where
        D: Deserializer<'de>,
    {
        let v: Vec<Datum> = Vec::deserialize(d)?;
