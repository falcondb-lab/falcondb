...
/// How a generated column's expression is stored and evaluated.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum GeneratedExprStorage {
    /// STORED: value is materialised at INSERT/UPDATE and persisted.
    /// Reads return the stored value directly — no recomputation.
    Stored,
    /// VIRTUAL: value is computed on every read, never stored.
    /// Saves storage space but adds CPU cost per read.
    Virtual,
}

/// A generated column expression (P2-1: Generated Columns).
///
/// Syntax (compatible with MySQL / PostgreSQL GENERATED ALWAYS AS):
/// ```sql
/// col INT GENERATED ALWAYS AS (a + b) STORED
/// col TEXT GENERATED ALWAYS AS (lower(name)) VIRTUAL
/// ```
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GeneratedColumnDef {
    /// SQL expression string (unparsed, evaluated at runtime by the executor).
    pub expr_sql: String,
    /// Whether the column is STORED or VIRTUAL.
    pub storage: GeneratedExprStorage,
}
...
pub struct ColumnDef {
    ...
    /// P2-1: Generated column expression.  None = regular column.
    /// If Some, this column's value is derived from other columns in the row.
    #[serde(default)]
...
impl ColumnDef {
    ...
    pub fn is_stored_generated(&self) -> bool {
        matches!(&self.generated, Some(g) if g.storage == GeneratedExprStorage::Stored)
    }

    /// Returns true if this is a VIRTUAL generated column.
    pub fn is_virtual_generated(&self) -> bool {
        matches!(&self.generated, Some(g) if g.storage == GeneratedExprStorage::Virtual)
    }
}
...
pub struct TableSchema {
    ...
    /// P2-1: Generated column indices — columns whose value is computed from
    /// other columns.  Maps column_index → GeneratedColumnDef.
    /// For STORED columns, the value is computed at INSERT/UPDATE and saved.
    /// For VIRTUAL columns, the value is computed at read time.
    #[serde(default)]
    pub generated_columns: std::collections::HashMap<usize, GeneratedColumnDef>,
    /// Partition specification for partition pruning (P1-7).
...
/// Partition spec for P1-7 partition pruning.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct TablePartitionSpec {
    /// Column indices used as the partition key (typically 1 column).
    pub columns: Vec<usize>,
    /// Partition kind: RANGE or LIST.
    pub kind: PartitionKind,
    /// For RANGE: ordered boundary values.  N boundaries = N+1 partitions.
    /// Partition i covers values in [bounds[i-1], bounds[i]).
    pub range_bounds: Vec<Datum>,
    /// For LIST: explicit value sets per partition.
    pub list_values: Vec<Vec<Datum>>,
}

/// Partition strategy kind.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Serialize, Deserialize)]
...
impl TablePartitionSpec {
    /// Return the partition ID for a given value (equality match).
    /// Returns None if no partition matches.
    pub fn partition_for_value(&self, val: &Datum) -> Option<u32> {
        match self.kind {
            PartitionKind::Range => {
                // Find the bucket index: partition i if bounds[i-1] <= val < bounds[i].
                for (i, bound) in self.range_bounds.iter().enumerate() {
                    if datum_lt(val, bound) {
                        return Some(i as u32);
                    }
                }
                Some(self.range_bounds.len() as u32) // last partition
            }
            PartitionKind::List => {
                for (i, values) in self.list_values.iter().enumerate() {
            ...
            PartitionKind::Hash => {
                let n = (self.range_bounds.len() + 1).max(1) as u32;
                Some(datum_hash(val) % n)
            }
        }
    }

    /// Return partition IDs that could contain values satisfying a range predicate.
    ///
    /// `op` is one of: `"<"`, `"<="`, `">"`, `">="`.
    /// Any other string returns all partition IDs (conservative / safe).
    pub fn partitions_for_range(&self, val: &Datum, op: &str) -> Vec<u32> {
        let n = (self.range_bounds.len() + 1) as u32;
        match op {
            "<" | "<=" => {
                // All partitions whose lower bound is less than val.
                let mut ids = Vec::new();
                for i in 0..n {
                    ids.push(i); // include all for now (conservative)
                    // Optimization: stop at first partition fully above val.
                    if i < self.range_bounds.len() as u32 {
                        let bound = &self.range_bounds[i as usize];
                        if datum_lt(val, bound) {
                            break; // remaining partitions are definitely above val
                        }
                    }
                }
                ids
            ...
            ">" | ">=" => {
                // All partitions whose upper bound is greater than val.
                let mut ids = Vec::new();
                ...
                for i in 0..n {
                    if !started {
                        if i == 0 {
                            ...
                            let prev = &self.range_bounds[(i - 1) as usize];
                            if datum_lt(prev, val) || prev == val {
                                ids.push(i);
                                started = true;
                            }
                        }
            ...
            _ => (0..n).collect(), // unknown op: conservative — include all
        }
    }
}

/// Simple datum less-than comparison for partition pruning.
fn datum_lt(a: &Datum, b: &Datum) -> bool {
    use std::cmp::Ordering;
    match (a, b) {
        (Datum::Int32(x), Datum::Int32(y)) => x < y,
        (Datum::Int64(x), Datum::Int64(y)) => x < y,
        (Datum::Float64(x), Datum::Float64(y)) => x < y,
...
/// Simple datum hash for HASH partitioning.
fn datum_hash(d: &Datum) -> u32 {
    use std::hash::{Hash, Hasher};
    let mut h = std::collections::hash_map::DefaultHasher::new();
    match d {
        Datum::Int32(v) => v.hash(&mut h),
        Datum::Int64(v) => v.hash(&mut h),
        Datum::Text(v) => v.hash(&mut h),
        Datum::Timestamp(v) => v.hash(&mut h),
        _ => 0u64.hash(&mut h),
    }
