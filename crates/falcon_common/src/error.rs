...
impl FalconError {
    ...
    pub fn pg_sqlstate(&self) -> &'static str {
        match self {
            ...
            FalconError::Storage(StorageError::TableAlreadyExists(_)) => "42P07",
            FalconError::Storage(StorageError::DuplicateKey) => "23505",
            FalconError::Storage(StorageError::UniqueViolation { .. }) => "23505",
            FalconError::Storage(StorageError::SerializationFailure) => "40001",
            FalconError::Txn(TxnError::WriteConflict(_)) => "40001",
            FalconError::Txn(TxnError::SerializationConflict(_)) => "40001",
            FalconError::Txn(TxnError::ConstraintViolation(_, _)) => "23000",
            FalconError::Txn(TxnError::InvariantViolation(_, _)) => "XX001",
            FalconError::Txn(TxnError::Aborted(_)) => "40000",
            FalconError::Sql(SqlError::Parse(_)) => "42601",
            FalconError::Sql(SqlError::UnknownTable(_)) => "42P01",
            FalconError::Sql(SqlError::UnknownColumn(_)) => "42703",
            FalconError::Sql(SqlError::TypeMismatch { .. }) => "42804",
            FalconError::Sql(SqlError::Unsupported(_)) => "0A000",
            FalconError::Sql(SqlError::ParamTypeRequired(_)) => "42P18",
            FalconError::Sql(SqlError::ParamTypeConflict { .. }) => "42P18",
            FalconError::Execution(ExecutionError::ParamMissing(_)) => "08P01",
            FalconError::Execution(ExecutionError::ParamTypeMismatch { .. }) => "42804",
            FalconError::Execution(ExecutionError::DivisionByZero) => "22012",
            FalconError::Storage(StorageError::MemoryPressure { .. }) => "53000",
