...
use cedar_common::types::{ShardId, TableId};
use cedar_sql_frontend::types::*;
use cedar_sql_frontend::types::SetOpKind;
...
pub enum PhysicalPlan {
    ...
    SeqScan {
        ...
        virtual_rows: Vec<cedar_common::datum::OwnedRow>,
    },
    ...
    Commit,
    Rollback,
    /// Observability: SHOW cedar_txn_stats
    ShowTxnStats,
    /// Observability: SHOW cedar.node_role
    ShowNodeRole,
    /// Observability: SHOW cedar.wal_stats
    ShowWalStats,
    /// Observability: SHOW cedar.connections
    ShowConnections,
    /// GC: SHOW cedar_gc
    RunGc,
    /// ANALYZE table_name — collect table statistics
    Analyze { table_name: String },
    /// SHOW cedar.table_stats [table_name]
    ShowTableStats { table_name: Option<String> },
    /// CREATE SEQUENCE
    CreateSequence { name: String, start: i64 },
    /// DROP SEQUENCE
    DropSequence { name: String, if_exists: bool },
    /// SHOW cedar.sequences
    ShowSequences,
...
/// How to merge a single aggregate column across shards (mirrors AggMerge in cedar_cluster).
#[derive(Debug, Clone)]
