...
use cedar_sql_frontend::types::{BoundExpr, ScalarFunc};

use super::binary_ops::eval_binary_op;
use super::cast::eval_cast;

// 鈹€鈹€ Domain dispatch router 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
fn try_dispatch_domain(func: &ScalarFunc, args: &[Datum]) -> Option<Result<Datum, ExecutionError>> {
    match func {
        ...
        ScalarFunc::Substring | ScalarFunc::Concat | ScalarFunc::ConcatWs |
        ScalarFunc::Trim | ScalarFunc::Btrim | ScalarFunc::Ltrim | ScalarFunc::Rtrim |
        ScalarFunc::Replace | ScalarFunc::Position |
        ScalarFunc::Lpad | ScalarFunc::Rpad | ScalarFunc::Left | ScalarFunc::Right |
        ScalarFunc::Repeat | ScalarFunc::Reverse | ScalarFunc::Initcap |
        ScalarFunc::Translate | ScalarFunc::Split | ScalarFunc::Overlay |
        ScalarFunc::StartsWith | ScalarFunc::EndsWith |
        ScalarFunc::Chr | ScalarFunc::Ascii |
        ScalarFunc::QuoteLiteral | ScalarFunc::QuoteIdent | ScalarFunc::QuoteNullable |
        ScalarFunc::BitLength | ScalarFunc::OctetLength =>
            Some(super::scalar_string::dispatch(func, args)),

        // Math domain
        ScalarFunc::Abs | ScalarFunc::Round | ScalarFunc::Ceil | ScalarFunc::Ceiling |
        ScalarFunc::Floor | ScalarFunc::Power | ScalarFunc::Sqrt | ScalarFunc::Sign |
        ScalarFunc::Trunc | ScalarFunc::Ln | ScalarFunc::Log | ScalarFunc::Exp |
        ScalarFunc::Pi | ScalarFunc::Mod | ScalarFunc::Degrees | ScalarFunc::Radians |
        ScalarFunc::Greatest | ScalarFunc::Least =>
            Some(super::scalar_math::dispatch(func, args)),

        // Time domain
        ScalarFunc::Now | ScalarFunc::CurrentDate | ScalarFunc::CurrentTime |
        ScalarFunc::Extract | ScalarFunc::DateTrunc | ScalarFunc::ToChar =>
            Some(super::scalar_time::dispatch(func, args)),

        // Array domain (core)
        ScalarFunc::ArrayLength | ScalarFunc::Cardinality |
        ScalarFunc::ArrayPosition | ScalarFunc::ArrayAppend | ScalarFunc::ArrayPrepend |
        ScalarFunc::ArrayRemove | ScalarFunc::ArrayReplace | ScalarFunc::ArrayCat |
        ScalarFunc::ArrayToString | ScalarFunc::StringToArray |
        ScalarFunc::ArrayUpper | ScalarFunc::ArrayLower | ScalarFunc::ArrayDims |
        ScalarFunc::ArrayReverse | ScalarFunc::ArrayDistinct | ScalarFunc::ArraySort |
        ScalarFunc::ArrayContains | ScalarFunc::ArrayOverlap =>
            Some(super::scalar_array::dispatch(func, args)),

        // Regex domain
        ScalarFunc::RegexpReplace | ScalarFunc::RegexpMatch |
        ScalarFunc::RegexpCount | ScalarFunc::RegexpSubstr |
        ScalarFunc::RegexpSplitToArray =>
            Some(super::scalar_regex::dispatch(func, args)),

        // Crypto/encoding domain
        ScalarFunc::Md5 | ScalarFunc::Sha256 |
        ScalarFunc::Encode | ScalarFunc::Decode | ScalarFunc::ToHex =>
            Some(super::scalar_crypto::dispatch(func, args)),

        // Utility domain
        ScalarFunc::PgTypeof | ScalarFunc::ToNumber |
        ScalarFunc::Random | ScalarFunc::GenRandomUuid |
        ScalarFunc::NumNonnulls | ScalarFunc::NumNulls =>
            Some(super::scalar_utility::dispatch(func, args)),

        // JSONB domain
        ScalarFunc::JsonbBuildObject | ScalarFunc::JsonbBuildArray |
        ScalarFunc::JsonbTypeof | ScalarFunc::JsonbArrayLength |
        ScalarFunc::JsonbExtractPath | ScalarFunc::JsonbExtractPathText |
        ScalarFunc::JsonbObjectKeys | ScalarFunc::JsonbPretty |
        ScalarFunc::JsonbStripNulls | ScalarFunc::JsonbSetPath |
        ScalarFunc::ToJsonb | ScalarFunc::JsonbAgg |
        ScalarFunc::JsonbConcat | ScalarFunc::JsonbDeleteKey |
        ScalarFunc::JsonbDeletePath | ScalarFunc::JsonbPopulateRecord |
        ...
        ScalarFunc::RowToJson =>
            Some(super::scalar_jsonb::dispatch(func, args)),
...
pub fn substitute_params_expr(expr: &BoundExpr, params: &[Datum]) -> Result<BoundExpr, ExecutionError> {
    match expr {
        ...
        | BoundExpr::SequenceSetval(_, _) | BoundExpr::Grouping(_) => Ok(expr.clone()),
        ...
        BoundExpr::Not(inner) => Ok(BoundExpr::Not(Box::new(substitute_params_expr(inner, params)?))),
        BoundExpr::IsNull(inner) => Ok(BoundExpr::IsNull(Box::new(substitute_params_expr(inner, params)?))),
        BoundExpr::IsNotNull(inner) => Ok(BoundExpr::IsNotNull(Box::new(substitute_params_expr(inner, params)?))),
        BoundExpr::IsNotDistinctFrom { left, right } => Ok(BoundExpr::IsNotDistinctFrom {
            left: Box::new(substitute_params_expr(left, params)?),
            right: Box::new(substitute_params_expr(right, params)?),
        }),
        BoundExpr::Like { expr, pattern, negated, case_insensitive } => Ok(BoundExpr::Like {
            expr: Box::new(substitute_params_expr(expr, params)?),
            pattern: Box::new(substitute_params_expr(pattern, params)?),
            negated: *negated,
            case_insensitive: *case_insensitive,
        }),
        BoundExpr::Between { expr, low, high, negated } => Ok(BoundExpr::Between {
            expr: Box::new(substitute_params_expr(expr, params)?),
            low: Box::new(substitute_params_expr(low, params)?),
            high: Box::new(substitute_params_expr(high, params)?),
            negated: *negated,
        }),
        BoundExpr::InList { expr, list, negated } => Ok(BoundExpr::InList {
            expr: Box::new(substitute_params_expr(expr, params)?),
            list: list.iter().map(|e| substitute_params_expr(e, params)).collect::<Result<_, _>>()?,
            negated: *negated,
        }),
        ...
        BoundExpr::Case { operand, conditions, results, else_result } => Ok(BoundExpr::Case {
            operand: operand.as_ref().map(|e| substitute_params_expr(e, params).map(Box::new)).transpose()?,
            conditions: conditions.iter().map(|e| substitute_params_expr(e, params)).collect::<Result<_, _>>()?,
            results: results.iter().map(|e| substitute_params_expr(e, params)).collect::<Result<_, _>>()?,
            else_result: else_result.as_ref().map(|e| substitute_params_expr(e, params).map(Box::new)).transpose()?,
        }),
        BoundExpr::Coalesce(args) => Ok(BoundExpr::Coalesce(
            args.iter().map(|e| substitute_params_expr(e, params)).collect::<Result<_, _>>()?,
        )),
        BoundExpr::Function { func, args } => Ok(BoundExpr::Function {
            func: func.clone(),
            args: args.iter().map(|e| substitute_params_expr(e, params)).collect::<Result<_, _>>()?,
        }),
        BoundExpr::AggregateExpr { func, arg, distinct } => Ok(BoundExpr::AggregateExpr {
            func: func.clone(),
            arg: arg.as_ref().map(|e| substitute_params_expr(e, params).map(Box::new)).transpose()?,
            distinct: *distinct,
        }),
        BoundExpr::ArrayLiteral(elems) => Ok(BoundExpr::ArrayLiteral(
            elems.iter().map(|e| substitute_params_expr(e, params)).collect::<Result<_, _>>()?,
        )),
        ...
        BoundExpr::AnyOp { left, compare_op, right } => Ok(BoundExpr::AnyOp {
            left: Box::new(substitute_params_expr(left, params)?),
            compare_op: *compare_op,
            right: Box::new(substitute_params_expr(right, params)?),
        }),
        BoundExpr::AllOp { left, compare_op, right } => Ok(BoundExpr::AllOp {
            left: Box::new(substitute_params_expr(left, params)?),
            compare_op: *compare_op,
            right: Box::new(substitute_params_expr(right, params)?),
        }),
        BoundExpr::ArraySlice { array, lower, upper } => Ok(BoundExpr::ArraySlice {
            array: Box::new(substitute_params_expr(array, params)?),
            lower: lower.as_ref().map(|e| substitute_params_expr(e, params).map(Box::new)).transpose()?,
            upper: upper.as_ref().map(|e| substitute_params_expr(e, params).map(Box::new)).transpose()?,
        }),
...
pub fn eval_expr_with_params(expr: &BoundExpr, row: &OwnedRow, params: &[Datum]) -> Result<Datum, ExecutionError> {
    match expr {
        ...
        BoundExpr::ArrayIndex { array, index } => {
            ...
            match (&arr_val, &idx_val) {
                ...
                (Datum::Null, _) | (_, Datum::Null) => Ok(Datum::Null),
                _ => Err(ExecutionError::TypeError("Array subscript requires array and integer index".into())),
            }
        }
        ...
        BoundExpr::Parameter(idx) => {
            ...
            if params.is_empty() {
                return Err(ExecutionError::TypeError(
                    format!("Parameter ${} must be bound before execution", idx),
                ));
            }
            let i = idx.checked_sub(1).ok_or({
                ExecutionError::ParamMissing(0)
            })?;
            params.get(i).cloned().ok_or({
                ExecutionError::ParamMissing(*idx)
            })
        }
        BoundExpr::AnyOp { left, compare_op, right } => {
            ...
            match rval {
                Datum::Array(ref elems) => {
                    ...
                    Ok(if has_null { Datum::Null } else { Datum::Boolean(false) })
                }
                Datum::Null => Ok(Datum::Null),
                _ => Err(ExecutionError::TypeError("ANY requires array operand".into())),
            }
        }
        BoundExpr::AllOp { left, compare_op, right } => {
            ...
            match rval {
                Datum::Array(ref elems) => {
                    ...
                    Ok(if has_null { Datum::Null } else { Datum::Boolean(true) })
                }
                Datum::Null => Ok(Datum::Null),
                _ => Err(ExecutionError::TypeError("ALL requires array operand".into())),
            }
        }
        BoundExpr::ArraySlice { array, lower, upper } => {
            ...
            match arr_val {
                    ...
                    let lo = match lower {
                        Some(e) => {
                            ...
                            match v {
                                ...
                                _ => return Err(ExecutionError::TypeError("Array slice bound must be integer".into())),
                            }
                        }
                        None => 1,
                    };
                ...
                Datum::Null => Ok(Datum::Null),
                _ => Err(ExecutionError::TypeError("Array slice requires array operand".into())),
            }
        }
...
pub fn eval_filter_with_params(expr: &BoundExpr, row: &OwnedRow, params: &[Datum]) -> Result<bool, ExecutionError> {
...
pub fn eval_having_expr(expr: &BoundExpr, group_rows: &[&OwnedRow]) -> Result<Datum, ExecutionError> {
    match expr {
        BoundExpr::AggregateExpr { func, arg, distinct } => {
            ...
            let distinct_vals = |e: &BoundExpr| -> Result<Vec<Datum>, ExecutionError> {
                ...
                for row in group_rows {
                    ...
                    let key = format!("{}", v);
                    if seen.insert(key) { vals.push(v); }
                }
                Ok(vals)
            };
            use cedar_sql_frontend::types::AggFunc;
            match func {
                AggFunc::Count => {
                    if let Some(e) = arg {
                        if *distinct { Ok(Datum::Int64(distinct_vals(e)?.len() as i64)) }
                        else { Ok(Datum::Int64(eval_all(e)?.len() as i64)) }
                    } else {
                        Ok(Datum::Int64(group_rows.len() as i64))
                    }
                }
                AggFunc::Sum => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("SUM requires arg".into()))?;
                    let vals = if *distinct { distinct_vals(e)? } else { eval_all(e)? };
                    let mut acc = Datum::Null;
                    for v in vals { acc = if acc.is_null() { v } else { acc.add(&v).unwrap_or(acc) }; }
                    Ok(acc)
                }
                AggFunc::Avg => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("AVG requires arg".into()))?;
                    let vals = if *distinct { distinct_vals(e)? } else { eval_all(e)? };
                    let mut sum = 0.0f64;
                    let mut count = 0i64;
                    for v in &vals { if let Some(f) = v.as_f64() { sum += f; count += 1; } }
                    if count == 0 { Ok(Datum::Null) } else { Ok(Datum::Float64(sum / count as f64)) }
                }
                AggFunc::Min => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("MIN requires arg".into()))?;
                    let vals = eval_all(e)?;
                    Ok(vals.into_iter().min().unwrap_or(Datum::Null))
                }
                AggFunc::Max => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("MAX requires arg".into()))?;
                    let vals = eval_all(e)?;
                    Ok(vals.into_iter().max().unwrap_or(Datum::Null))
                }
                AggFunc::BoolAnd => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("BOOL_AND requires arg".into()))?;
                    let vals = eval_all(e)?;
                    if vals.is_empty() { Ok(Datum::Null) }
                    else { Ok(Datum::Boolean(vals.iter().all(|v| v.as_bool().unwrap_or(true)))) }
                }
                AggFunc::BoolOr => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("BOOL_OR requires arg".into()))?;
                    let vals = eval_all(e)?;
                    if vals.is_empty() { Ok(Datum::Null) }
                    else { Ok(Datum::Boolean(vals.iter().any(|v| v.as_bool().unwrap_or(false)))) }
                }
                AggFunc::StringAgg(sep) => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("STRING_AGG requires arg".into()))?;
                    let vals = eval_all(e)?;
                    if vals.is_empty() { Ok(Datum::Null) }
                    else { Ok(Datum::Text(vals.iter().map(|v| format!("{}", v)).collect::<Vec<_>>().join(sep))) }
                }
                AggFunc::ArrayAgg => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("ARRAY_AGG requires arg".into()))?;
                    let vals = if *distinct { distinct_vals(e)? } else { eval_all(e)? };
                    if vals.is_empty() { Ok(Datum::Null) } else { Ok(Datum::Array(vals)) }
                }
                // Statistical aggregates in HAVING — compute inline
                AggFunc::VarPop | AggFunc::VarSamp | AggFunc::StddevPop | AggFunc::StddevSamp => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("Statistical aggregate requires arg".into()))?;
                    let vals = if *distinct { distinct_vals(e)? } else { eval_all(e)? };
                    ...
                    let variance = match func {
                        ...
                        _ => { if n < 2 { return Ok(Datum::Null); } sum_sq / (n - 1) as f64 }
                    };
                    match func {
                ...
                AggFunc::Mode => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("MODE requires arg".into()))?;
                    let vals = eval_all(e)?;
                    if vals.is_empty() { return Ok(Datum::Null); }
                    let mut freq: Vec<(String, usize, Datum)> = Vec::new();
                    for v in vals {
                        let key = format!("{}", v);
                        if let Some(entry) = freq.iter_mut().find(|(k, _, _)| *k == key) { entry.1 += 1; }
                        else { freq.push((key, 1, v)); }
                    }
                    freq.sort_by(|a, b| b.1.cmp(&a.1));
                    Ok(freq.into_iter().next().map(|(_, _, v)| v).unwrap_or(Datum::Null))
                }
                AggFunc::BitAndAgg => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("BIT_AND requires arg".into()))?;
                    let vals = eval_all(e)?;
                    if vals.is_empty() { return Ok(Datum::Null); }
                    let mut r: Option<i64> = None;
                    for v in &vals { if let Some(i) = v.as_i64() { r = Some(r.map_or(i, |acc| acc & i)); } }
                    Ok(r.map(Datum::Int64).unwrap_or(Datum::Null))
                }
                AggFunc::BitOrAgg => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("BIT_OR requires arg".into()))?;
                    let vals = eval_all(e)?;
                    if vals.is_empty() { return Ok(Datum::Null); }
                    let mut r: i64 = 0;
                    for v in &vals { if let Some(i) = v.as_i64() { r |= i; } }
                    Ok(Datum::Int64(r))
                }
                AggFunc::BitXorAgg => {
                    let e = arg.as_ref().ok_or(ExecutionError::TypeError("BIT_XOR requires arg".into()))?;
                    let vals = eval_all(e)?;
                    if vals.is_empty() { return Ok(Datum::Null); }
                    let mut r: i64 = 0;
                    for v in &vals { if let Some(i) = v.as_i64() { r ^= i; } }
...
pub fn eval_having_filter(expr: &BoundExpr, group_rows: &[&OwnedRow]) -> Result<bool, ExecutionError> {
