...
use cedar_sql_frontend::types::BoundOrderBy;
...
impl ExternalSorter {
    ...
    pub fn from_config(config: &SpillConfig) -> Option<Self> {
        ...
        let temp_dir = if config.temp_dir.is_empty() {
            std::env::temp_dir().join("cedar_spill")
        } else {
    ...
    fn create_run_dir(&self) -> Result<PathBuf, CedarError> {
        ...
        fs::create_dir_all(&dir).map_err(|e| {
            CedarError::Internal(format!("Failed to create spill directory {:?}: {}", dir, e))
        })?;
        Ok(dir)
    }
}
...
fn write_run(path: &Path, rows: &[OwnedRow]) -> Result<(), CedarError> {
    let file = File::create(path).map_err(|e| {
        CedarError::Internal(format!("Failed to create spill file {:?}: {}", path, e))
    })?;
    ...
    for row in rows {
        let bytes = bincode::serialize(row).map_err(|e| {
            CedarError::Internal(format!("Spill serialization error: {}", e))
        })?;
...
fn read_run(path: &Path) -> Result<Vec<OwnedRow>, CedarError> {
    let file = File::open(path).map_err(|e| {
        CedarError::Internal(format!("Failed to open spill file {:?}: {}", path, e))
    })?;
...
impl RunReader {
    fn open(path: &Path) -> Result<Self, CedarError> {
        let file = File::open(path).map_err(|e| {
            CedarError::Internal(format!("Failed to open spill file {:?}: {}", path, e))
        })?;
    ...
    fn next_row(&mut self) -> Result<Option<OwnedRow>, CedarError> {
        if self.remaining == 0 {
            return Ok(None);
        }
        self.remaining -= 1;
...
fn merge_runs(
    ...
    let file = File::create(output_path).map_err(|e| {
        CedarError::Internal(format!("Failed to create merge file {:?}: {}", output_path, e))
    })?;
    let mut w = BufWriter::new(file);
...
fn read_u64(r: &mut impl Read) -> Result<u64, CedarError> {
    let mut buf = [0u8; 8];
    r.read_exact(&mut buf).map_err(io_err)?;
    Ok(u64::from_le_bytes(buf))
}

fn read_one_row(r: &mut impl Read) -> Result<OwnedRow, CedarError> {
    ...
    bincode::deserialize(&data).map_err(|e| {
        CedarError::Internal(format!("Spill deserialization error: {}", e))
    })
}

fn io_err(e: std::io::Error) -> CedarError {
    CedarError::Internal(format!("Spill I/O error: {}", e))
}
...
mod tests {
    ...
    fn test_write_read_run_roundtrip() {
        ...
        let dir = std::env::temp_dir().join("cedar_test_run");
