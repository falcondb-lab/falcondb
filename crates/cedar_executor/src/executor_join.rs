...
impl Executor {
    pub(crate) fn exec_nested_loop_join(
        &self,
        left_table_id: cedar_common::types::TableId,
        ...
        for join in joins {
            let right_data: Vec<OwnedRow> = if let Some(cte_rows) = cte_data.get(&join.right_table_id) {
                cte_rows.clone()
            } else {
                let right_rows = self.storage.scan(join.right_table_id, txn.txn_id, read_ts)?;
                    ...
                    for left_row in &combined_rows {
                        for right_row in &right_data {
                            ...
                            if let Some(ref cond) = join.condition {
                                if !ExprEngine::eval_filter(cond, &merged).map_err(CedarError::Execution)? {
                                    continue;
                                }
                            }
                ...
                JoinType::Left => {
                    for left_row in &combined_rows {
                        ...
                        if !matched {
                            ...
                            let null_right = OwnedRow::new(vec![Datum::Null; join.right_schema.columns.len()]);
    ...
    pub(crate) fn exec_hash_join(
        ...
        let left_data: Vec<OwnedRow> = if let Some(cte_rows) = cte_data.get(&left_table_id) {
            ...
            self.storage.scan(left_table_id, txn.txn_id, read_ts)?
                .into_iter().map(|(_, row)| row).collect()
        };
        ...
        for join in joins {
            let right_data: Vec<OwnedRow> = if let Some(cte_rows) = cte_data.get(&join.right_table_id) {
                ...
                self.storage.scan(join.right_table_id, txn.txn_id, read_ts)?
                    .into_iter().map(|(_, row)| row).collect()
            };
            ...
            if key_pairs.is_empty() {
                ...
                for (ri, right_row) in right_data.iter().enumerate() {
                    let key: Vec<Datum> = key_pairs.iter()
                ...
                match join.join_type {
                    JoinType::Inner => {
                        for left_row in &combined_rows {
                            let probe_key: Vec<Datum> = key_pairs.iter()
    ...
    pub(crate) fn exec_merge_sort_join(
        ...
        for join in joins {
            ...
            if key_pairs.is_empty() || !matches!(join.join_type, JoinType::Inner) {
                ...
                let mut left_sorted: Vec<(Vec<Datum>, usize)> = combined_rows.iter().enumerate()
                    .map(|(i, row)| {
                        let key: Vec<Datum> = key_pairs.iter()
                ...
                let mut right_sorted: Vec<(Vec<Datum>, usize)> = right_data.iter().enumerate()
                    .map(|(i, row)| {
                        let key: Vec<Datum> = key_pairs.iter()
                    ...
                    match cmp {
                        ...
                        std::cmp::Ordering::Equal => {
                            ...
                            while left_group_end < left_sorted.len()
                                && Self::cmp_datum_keys(&left_sorted[left_group_end].0, &left_key) == std::cmp::Ordering::Equal {
                                left_group_end += 1;
                            }
                            ...
                            while right_group_end < right_sorted.len()
                                && Self::cmp_datum_keys(&right_sorted[right_group_end].0, &right_key) == std::cmp::Ordering::Equal {
                                right_group_end += 1;
                            }
