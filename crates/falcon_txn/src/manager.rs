...
/// Cache-line padded wrapper to prevent false sharing between adjacent atomic counters.
/// Each wrapped value occupies its own 64-byte cache line, so concurrent writes
/// from different cores to different counters do not invalidate each other's lines.
#[repr(align(64))]
...
/// Drain threshold for the per-thread latency/history record buffer.
/// When a thread accumulates this many TxnRecords locally, it flushes
/// them into the global Mutex<LatencyRecorder> and Mutex<TxnHistory> in
/// a single lock acquisition — reducing contention by RECORD_DRAIN_BATCH×.
const RECORD_DRAIN_BATCH: usize = 32;

/// Per-thread pending latency sample: (TxnPath, priority, latency_us).
#[derive(Clone)]
...
/// Per-thread record drain buffer.
struct ThreadRecordBuf {
    latency: Vec<PendingLatency>,
    history: Vec<TxnRecord>,
}

impl ThreadRecordBuf {
    fn new() -> Self {
        Self {
            latency: Vec::with_capacity(RECORD_DRAIN_BATCH),
            history: Vec::with_capacity(RECORD_DRAIN_BATCH),
        }
    }
    fn is_full(&self) -> bool {
        self.latency.len() >= RECORD_DRAIN_BATCH || self.history.len() >= RECORD_DRAIN_BATCH
    }
}

thread_local! {
    static TLS_RECORD_BUF: std::cell::RefCell<ThreadRecordBuf> =
        std::cell::RefCell::new(ThreadRecordBuf::new());
}

/// Batch size for thread-local timestamp lease.
/// Each worker thread pre-fetches this many timestamps with a single
/// global LOCK XADD, reducing atomic contention by 64× at 32-core load.
const TS_LEASE_BATCH: u64 = 64;

/// Per-thread timestamp lease: owns the half-open range [next, end).
/// When exhausted, the thread performs one global fetch_add(BATCH) to refill.
#[derive(Clone, Copy)]
struct TsLease {
    next: u64,
    end: u64,
...
thread_local! {
    static TS_LEASE: std::cell::Cell<TsLease> =
        std::cell::Cell::new(TsLease::empty());
}
...
use falcon_storage::engine::{set_txn_read_committed, StorageEngine};

use crate::deadlock::{SsiLockManager, SsiWriteIntent};
use crate::lock_manager::{LockManager, LockMode, RowKey, TxnLockSet};
...
pub struct TxnHandle {
    ...
    pub path: TxnPath,
    pub slow_path_mode: SlowPathMode,
    pub involved_shards: SmallVec<[ShardId; 2]>,
    pub degraded: bool,
    pub state: TxnState,
    ...
    /// P0-1: Pessimistic row-level locks held by this transaction.
    pub lock_set: TxnLockSet,
    /// P1-1: Named savepoints for SAVEPOINT / ROLLBACK TO SAVEPOINT.
    pub savepoints: Vec<SavepointEntry>,
}

/// A single savepoint within a transaction (P1-1).
#[derive(Debug, Clone)]
pub struct SavepointEntry {
    /// User-visible savepoint name.
    pub name: String,
    /// Length of the storage write-set at the time the savepoint was created.
    /// Used to truncate the write-set on ROLLBACK TO SAVEPOINT.
...
impl TxnHandle {
    ...
    pub fn to_context(&self) -> TxnContext {
        TxnContext {
            ...
            txn_path: self.path,
            involved_shards: self.involved_shards.to_vec(),
...
/// Each field is wrapped in CachePadded to occupy its own cache line,
/// eliminating false sharing when different cores increment different counters.
struct TxnStatsCollector {
    total_committed: CachePadded<AtomicU64>,
    fast_path_commits: CachePadded<AtomicU64>,
    slow_path_commits: CachePadded<AtomicU64>,
    total_aborted: CachePadded<AtomicU64>,
    occ_conflicts: CachePadded<AtomicU64>,
    degraded_to_global: CachePadded<AtomicU64>,
    constraint_violations: CachePadded<AtomicU64>,
    admission_rejections: CachePadded<AtomicU64>,
    /// Number of times an implicit fast→slow path upgrade was blocked.
    implicit_upgrade_blocked: CachePadded<AtomicU64>,
}

impl TxnStatsCollector {
    const fn new() -> Self {
        Self {
            total_committed: CachePadded::new(AtomicU64::new(0)),
            fast_path_commits: CachePadded::new(AtomicU64::new(0)),
            slow_path_commits: CachePadded::new(AtomicU64::new(0)),
            total_aborted: CachePadded::new(AtomicU64::new(0)),
            occ_conflicts: CachePadded::new(AtomicU64::new(0)),
            degraded_to_global: CachePadded::new(AtomicU64::new(0)),
            constraint_violations: CachePadded::new(AtomicU64::new(0)),
            admission_rejections: CachePadded::new(AtomicU64::new(0)),
            implicit_upgrade_blocked: CachePadded::new(AtomicU64::new(0)),
        }
    }
...
pub struct TxnManager {
    ...
    ssi: SsiLockManager,
    /// P0-1: Pessimistic row-level lock manager.
    lock_mgr: LockManager,
}

impl TxnManager {
    pub fn new(storage: Arc<StorageEngine>) -> Self {
        let wfg = std::sync::Arc::new(crate::deadlock::WaitForGraph::new());
        Self {
    ...
    /// On the non-HLC fast path, uses a thread-local lease to batch
    /// TS_LEASE_BATCH allocations into a single global LOCK XADD.
    pub fn alloc_ts(&self) -> Timestamp {
        if let Some(ref hlc) = self.hlc {
            let ts = hlc.now();
            // Keep monotonic counter in sync so current_ts() stays valid.
            // Also invalidate any thread-local lease since HLC may jump forward.
            self.ts_counter.fetch_max(ts.0 + 1, Ordering::Relaxed);
            TS_LEASE.with(|cell| cell.set(TsLease::empty()));
            return ts;
        }
        // Non-HLC fast path: drain thread-local lease before touching global counter.
        TS_LEASE.with(|cell| {
            let mut lease = cell.get();
            ...
            // Lease exhausted — refill with a single global fetch_add.
            let base = self.ts_counter.fetch_add(TS_LEASE_BATCH, Ordering::Relaxed);
            cell.set(TsLease { next: base + 1, end: base + TS_LEASE_BATCH });
            Timestamp(base)
        })
    }

    /// Invalidate the calling thread's timestamp lease.
    /// Must be called after advance_counters_past() or any out-of-band ts bump
    /// to prevent a thread from handing out timestamps below the new floor.
    #[inline]
    fn invalidate_ts_lease() {
        TS_LEASE.with(|cell| cell.set(TsLease::empty()));
    }
    ...
    pub fn advance_counters_past(&self, max_ts: u64, max_txn_id: u64) {
        if max_ts > 0 {
            ...
            // Invalidate calling thread's lease so it re-fetches above the new floor.
            Self::invalidate_ts_lease();
        }
        if max_txn_id > 0 {
            self.txn_counter
    ...
    pub fn begin_with_classification(
        ...
        let handle = TxnHandle {
            ...
            involved_shards: SmallVec::from_vec(classification.involved_shards),
            degraded: false,
            ...
            lock_set: TxnLockSet::new(),
            savepoints: Vec::new(),
        };

        self.active_txns.insert(txn_id, handle.clone());
        // P0-A: RC transactions never validate read-sets — skip pk.to_vec() per read.
        if isolation == IsolationLevel::ReadCommitted {
            set_txn_read_committed(true);
        }
        handle
    }
    ...
    pub fn begin_at_snapshot_on_shard(
        ...
        let handle = TxnHandle {
            ...
            path: TxnPath::Fast,
            slow_path_mode: SlowPathMode::Xa2Pc,
            involved_shards: smallvec::smallvec![shard_id],
            degraded: false,
            state: TxnState::Active,
            begin_instant: Some(Instant::now()),
            trace_id: txn_id.0,
            ...
            timeout_ms: 0,
            exec_summary: TxnExecSummary::default(),
            lock_set: crate::lock_manager::TxnLockSet::new(),
            savepoints: Vec::new(),
        };
        self.active_txns.insert(txn_id, handle.clone());
        handle
    ...
    pub fn commit(&self, txn_id: TxnId) -> Result<Timestamp, TxnError> {
        ...
        if entry.txn_type == TxnType::Local && entry.involved_shards.len() != 1 {
            tracing::error!(txn_id = txn_id.0, shards = entry.involved_shards.len(),
                "TXN invariant violation: LocalTxn wrong shard count");
            return Err(TxnError::InvariantViolation(
                txn_id,
                "LocalTxn must have exactly 1 involved shard".into(),
            ));
        }
        if entry.txn_type == TxnType::Local && entry.path == TxnPath::Slow {
            tracing::error!(txn_id = txn_id.0,
                "TXN invariant violation: LocalTxn on slow path");
            return Err(TxnError::InvariantViolation(
                txn_id,
                "LocalTxn must be on fast path".into(),
            ));
        }
        if entry.txn_type == TxnType::Global && entry.path == TxnPath::Fast {
            tracing::error!(txn_id = txn_id.0,
                "TXN invariant violation: GlobalTxn on fast path");
            return Err(TxnError::InvariantViolation(
                txn_id,
                "GlobalTxn must be on slow path".into(),
            ));
        }
        ...
        if local_fast_path {
            ...
            self.active_txns.remove(&txn_id);
            // P1-A: Only pay for latency measurement + TxnRecord construction when
            // the record will actually be consumed (sampled or slow-txn threshold hit).
            let divisor = self.latency_sample_divisor;
            let threshold = self.slow_txn_threshold_us.load(Ordering::Relaxed);
            let will_sample = divisor <= 1 || txn_id.0.is_multiple_of(divisor);
            if will_sample || threshold > 0 {
                let latency_us =
                    entry_begin_instant.map_or(0, |i| i.elapsed().as_micros() as u64);
                let is_slow = threshold > 0 && latency_us >= threshold;
                if will_sample || is_slow {
                    self.record_completed(TxnRecord {
                        txn_id,
                        txn_type: TxnType::Local,
                        txn_path: TxnPath::Fast,
            ...
            // P0-1: release all pessimistic row locks on fast-path commit
            if let Some(mut entry) = self.active_txns.get_mut(&txn_id) {
                let keys = entry.lock_set.drain();
                drop(entry);
                self.lock_mgr.release_all(txn_id, &keys);
            }
            set_txn_read_committed(false);
            self.stats.record_fast_commit();
            return Ok(commit_ts);
        }
        ...
        // P0-1: release pessimistic row locks on slow-path commit
        if let Some(mut entry) = self.active_txns.get_mut(&txn_id) {
            let keys = entry.lock_set.drain();
            drop(entry);
            self.lock_mgr.release_all(txn_id, &keys);
    ...
    pub fn abort_with_reason(&self, txn_id: TxnId, reason: &str) -> Result<(), TxnError> {
        ...
        // P0-1: release all pessimistic row locks on abort
        if let Some(mut entry) = self.active_txns.get_mut(&txn_id) {
            let keys = entry.lock_set.drain();
            drop(entry);
            self.lock_mgr.release_all(txn_id, &keys);
    ...
    /// P0-1 / P0-2: Acquire a pessimistic row-level lock on behalf of a transaction.
    ///
    /// Used by `SELECT FOR UPDATE` (Exclusive), `SELECT FOR SHARE` (Share),
    /// and hot-row write serialization.
    ///
    /// The acquired key is added to the transaction's `lock_set` so it is
    /// released automatically on commit or abort.
    pub fn acquire_row_lock(
        ...
        let key = RowKey::new(table_id, pk);
        self.lock_mgr.acquire(txn_id, &key, mode, timeout_ms)?;
        // Register key in the transaction's lock_set for cleanup
        if let Some(mut entry) = self.active_txns.get_mut(&txn_id) {
            entry.lock_set.add(key);
        }
        Ok(())
    }

    /// P0-1: Snapshot of the lock table for `SHOW falcon.lock_stats`.
    pub fn lock_table_snapshot(&self) -> crate::lock_manager::LockTableSnapshot {
        self.lock_mgr.snapshot()
    }

    /// P1-1: Create a named savepoint within an active transaction.
    ///
    /// Records the current write-set length so `ROLLBACK TO SAVEPOINT` can
    /// truncate the write set back to this point.
    pub fn savepoint(&self, txn_id: TxnId, name: String) -> Result<(), TxnError> {
        let mut entry = self
            .active_txns
            .get_mut(&txn_id)
            .ok_or(TxnError::NotFound(txn_id))?;
        ...
        // Remove any existing savepoint with same name (PG semantics: re-definition)
        entry.savepoints.retain(|s| s.name != name);
        let write_set_len = self.storage.write_set_snapshot(txn_id);
        entry.savepoints.push(SavepointEntry {
            name,
            write_set_len,
        });
        Ok(())
    ...
    /// P1-1: Roll back to a previously defined savepoint.
    ///
    /// Truncates the transaction write-set to the saved length, effectively
    /// undoing all writes since the savepoint was created.  Savepoints defined
    /// *after* the target are also removed (PG semantics).
    pub fn rollback_to_savepoint(&self, txn_id: TxnId, name: &str) -> Result<(), TxnError> {
        ...
        // Drop all savepoints defined after this one
        let idx = entry.savepoints.iter().position(|s| s.name == name).unwrap();
        entry.savepoints.truncate(idx + 1);
        drop(entry);
        // Truncate the write set in storage
        self.storage.rollback_write_set_after(txn_id, sp.write_set_len);
        Ok(())
    }

    /// P1-1: Release (destroy) a named savepoint without rolling back.
    pub fn release_savepoint(&self, txn_id: TxnId, name: &str) -> Result<(), TxnError> {
        let mut entry = self
            .active_txns
            .get_mut(&txn_id)
            .ok_or(TxnError::NotFound(txn_id))?;
        let before = entry.savepoints.len();
        entry.savepoints.retain(|s| s.name != name);
        if entry.savepoints.len() == before {
            return Err(TxnError::InvariantViolation(
                txn_id,
                format!("savepoint \"{name}\" does not exist"),
            ));
    ...
    /// Flushes the calling thread's TLS record buffer first so that any
    /// buffered samples (up to RECORD_DRAIN_BATCH) are included.
    pub fn stats_snapshot(&self) -> TxnStatsSnapshot {
        self.flush_tls_record_buf();
    ...
    fn record_completed(&self, record: TxnRecord) {
        ...
        // Slow transactions bypass the TLS buffer and flush immediately so that
        // the slow_txn_log reflects reality with minimal delay.
        if is_slow {
            tracing::warn!(
            ...
            // Also route into history and latency immediately.
        ...
        // P0-2: Buffer sampled (non-slow) records in a per-thread VecDeque.
        // Drain into the global Mutex structures only when the buffer fills
        // (every RECORD_DRAIN_BATCH commits), reducing contention by RECORD_DRAIN_BATCH×.
        let should_drain = TLS_RECORD_BUF.with(|cell| {
            let mut buf = cell.borrow_mut();
            if record.outcome == TxnOutcome::Committed {
                buf.latency.push(PendingLatency {
                    path: record.txn_path,
                    priority: record.priority,
                    latency_us: record.commit_latency_us,
                });
            }
            buf.history.push(record);
            buf.is_full()
        });
    ...
    /// Drain the calling thread's TLS record buffer into the global Mutex structures.
    /// Called when the buffer fills, and may be called externally to force a flush
    /// (e.g. before stats_snapshot to ensure fresh data).
    fn flush_tls_record_buf(&self) {
        TLS_RECORD_BUF.with(|cell| {
            let mut buf = cell.borrow_mut();
            if buf.latency.is_empty() && buf.history.is_empty() {
                return;
            }
            // Drain latency samples — single lock acquisition for the whole batch.
            if !buf.latency.is_empty() {
                let mut lat = self.latency.lock();
                for s in buf.latency.drain(..) {
                    lat.record(s.path, s.latency_us);
                    lat.record_priority(s.priority, s.latency_us);
                }
            }
            // Drain history records — single lock acquisition for the whole batch.
            if !buf.history.is_empty() {
                let mut hist = self.history.lock();
                for r in buf.history.drain(..) {
                    hist.push(r);
                }
            }
        });
