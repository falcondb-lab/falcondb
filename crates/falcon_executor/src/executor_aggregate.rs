...
impl Executor {
    ...
    pub(crate) fn exec_fused_aggregate(
        ...
        // ── GROUP BY: hash-aggregate with spill-to-disk (E3) ──
        //
        // When the number of in-memory groups exceeds hash_agg_group_limit and a
        // spill directory is configured, the current partition is flushed to a
        // temporary file (one file per hash partition) and the in-memory map is
        // cleared.  After all rows are processed the spill files are re-read and
        // merged into the final result.  This turns an OOM hard-error into a
        // bounded-memory operation at the cost of extra I/O.
        struct PartialGroupBy {
            groups: HashMap<Vec<u8>, (Vec<ProjAccum>, bool)>,
            key_buf: Vec<u8>,
            error: Option<FalconError>,
            group_limit: usize,
            // Spill state (None = spill disabled)
            spill_dir: Option<PathBuf>,
            spill_files: Vec<PathBuf>,
            spill_seq: u64,
        }

        // Helper: flush in-memory groups to a spill file.
        fn flush_groups_to_spill(
            groups: &mut HashMap<Vec<u8>, (Vec<ProjAccum>, bool)>,
            spill_dir: &Path,
            seq: &mut u64,
            files: &mut Vec<PathBuf>,
        ) -> Result<(), FalconError> {
            fs::create_dir_all(spill_dir).map_err(|e| {
                FalconError::Internal(format!("hash-agg spill mkdir failed: {e}"))
            })?;
            let path = spill_dir.join(format!("hagg_{:06}.bin", *seq));
            *seq += 1;
            let file = fs::File::create(&path).map_err(|e| {
                FalconError::Internal(format!("hash-agg spill create failed: {e}"))
            })?;
            let mut w = BufWriter::new(file);
            // Format: [group_count: u64] then for each group:
            //   [key_len: u32][key bytes][accum_count: u32][bincode(ProjAccum)*]
            w.write_all(&(groups.len() as u64).to_le_bytes())
                .map_err(|e| FalconError::Internal(format!("hagg spill write: {e}")))?;
            let mut ser_buf: Vec<u8> = Vec::with_capacity(256);
            for (key, (accums, first)) in groups.iter() {
                // key
                w.write_all(&(key.len() as u32).to_le_bytes())
                    .map_err(|e| FalconError::Internal(format!("hagg spill key_len: {e}")))?;
                w.write_all(key)
                    .map_err(|e| FalconError::Internal(format!("hagg spill key: {e}")))?;
                // accums serialized as OwnedRow of Datum results
                let row_vals: Vec<Datum> = accums.iter().map(ProjAccum::result).collect();
                let row = OwnedRow::new(row_vals);
                ser_buf.clear();
                bincode::serialize_into(&mut ser_buf, &row).map_err(|e| {
                    FalconError::Internal(format!("hagg spill bincode: {e}"))
                })?;
                w.write_all(&(ser_buf.len() as u32).to_le_bytes())
                    .map_err(|e| FalconError::Internal(format!("hagg spill row_len: {e}")))?;
                w.write_all(&ser_buf)
                    .map_err(|e| FalconError::Internal(format!("hagg spill row: {e}")))?;
                // first flag
                w.write_all(&[u8::from(*first)])
                    .map_err(|e| FalconError::Internal(format!("hagg spill first: {e}")))?;
            }
            w.flush()
                .map_err(|e| FalconError::Internal(format!("hagg spill flush: {e}")))?;
            groups.clear();
            files.push(path);
            Ok(())
        }
        ...
        let spill_dir_opt = self.hash_agg_spill_dir.clone();
        let result = self.storage.map_reduce_visible(
            table_id,
            txn.txn_id,
            read_ts,
            ...
            |state, row| {
                ...
                let (accums, is_first) =
                    if let Some(entry) = state.groups.get_mut(state.key_buf.as_slice()) {
                        ...
                        // Group count exceeded — spill if directory configured, else error.
                        if state.group_limit > 0 && state.groups.len() >= state.group_limit {
                            if let Some(ref dir) = state.spill_dir {
                                if let Err(e) = flush_groups_to_spill(
                                    &mut state.groups,
                                    dir,
                                    &mut state.spill_seq,
                                ...
                                state.error = Some(FalconError::Execution(
                                    ExecutionError::ResourceExhausted(format!(
                                        "hash aggregation group limit exceeded: {} groups \
                                         (limit: {}). Configure spill directory or increase limit.",
                                        state.groups.len(),
                                        state.group_limit,
                                    )),
                                ));
                        ...
                        state.groups.get_mut(state.key_buf.as_slice())
                            .expect("BUG: key missing immediately after insert")
                    };
            ...
            |mut a, b| {
                ...
                // Merge spill file lists
                a.spill_files.extend(b.spill_files);
                a
            },
        )?;
        if let Some(e) = result.error {
        ...
        // Merge in-memory groups with any spilled partitions.
        let mut groups = result.groups;
        for spill_path in &result.spill_files {
            let file = fs::File::open(spill_path).map_err(|e| {
                FalconError::Internal(format!("hagg spill re-open: {e}"))
            })?;
            let mut r = BufReader::new(file);
            let mut cnt_buf = [0u8; 8];
            r.read_exact(&mut cnt_buf)
                .map_err(|e| FalconError::Internal(format!("hagg spill read cnt: {e}")))?;
            let count = u64::from_le_bytes(cnt_buf) as usize;
            for _ in 0..count {
                // key
                let mut klen_buf = [0u8; 4];
                r.read_exact(&mut klen_buf)
                    .map_err(|e| FalconError::Internal(format!("hagg spill read klen: {e}")))?;
                let klen = u32::from_le_bytes(klen_buf) as usize;
                let mut key = vec![0u8; klen];
                r.read_exact(&mut key)
                    .map_err(|e| FalconError::Internal(format!("hagg spill read key: {e}")))?;
                // row
                let mut rlen_buf = [0u8; 4];
                r.read_exact(&mut rlen_buf)
                    .map_err(|e| FalconError::Internal(format!("hagg spill read rlen: {e}")))?;
                let rlen = u32::from_le_bytes(rlen_buf) as usize;
                let mut data = vec![0u8; rlen];
                r.read_exact(&mut data)
                    .map_err(|e| FalconError::Internal(format!("hagg spill read row: {e}")))?;
                let spilled_row: OwnedRow = bincode::deserialize(&data).map_err(|e| {
                    FalconError::Internal(format!("hagg spill deser: {e}"))
                })?;
                // first flag
                let mut first_buf = [0u8; 1];
                r.read_exact(&mut first_buf)
                    .map_err(|e| FalconError::Internal(format!("hagg spill read first: {e}")))?;
                let spilled_first = first_buf[0] != 0;
                // Re-feed the spilled partial result back into the groups map.
                let entry = groups.entry(key).or_insert_with(|| {
                    (Self::create_accums(projections), true)
                });
                for (acc, spilled_val) in entry.0.iter_mut().zip(spilled_row.values.iter()) {
                    if !spilled_val.is_null() {
                        acc.feed_value(spilled_val);
                    }
                }
                entry.1 = entry.1 && spilled_first;
            }
