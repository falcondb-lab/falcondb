...
use falcon_storage::memtable::{encode_column_value, encode_pk_from_datums_stack, GinSearchTerm, PkBuf, SimpleAggOp};
...
use crate::vectorized::{is_vectorizable, vectorized_filter, vectorized_project, RecordBatch};
...
impl Executor {
    ...
    pub(crate) fn try_pk_point_lookup(
        ...
        let datum_refs: Vec<&Datum> = pk_datums
            ...
            .collect();
        let pk: PkBuf = encode_pk_from_datums_stack(&datum_refs);
    ...
    pub(crate) fn exec_seq_scan(
        ...
        let (raw_rows, effective_filter): (Vec<(Vec<u8>, OwnedRow)>, Option<BoundExpr>) =
            ...
            } else if let Some((pk_buf, remaining)) = self.try_pk_point_lookup(filter, schema) {
                // PK point lookup: O(1) hash lookup instead of full scan.
                // pk_buf is SmallVec<[u8;16]> — zero heap alloc for the lookup.
                let read_ts = txn.read_ts(self.txn_mgr.current_ts());
                match self.storage.get(table_id, &pk_buf, txn.txn_id, read_ts)? {
                    Some(row) => (vec![(pk_buf.to_vec(), row)], remaining),
                    None => (vec![], remaining),
                }
        ...
        if let Some(ref f) = mat_filter {
                ...
                && is_vectorizable(projections, mat_filter.as_ref())
            {
                ...
                // ── Vectorized filter+project path (single-threaded, batched) ──
                let num_cols = schema.columns.len();
                let mut batch = RecordBatch::from_row_pairs(&raw_rows, num_cols);
                vectorized_filter(&mut batch, f);
                if !has_window {
                    // Fast path: project in-batch without materialising individual rows.
                    let projected =
                        vectorized_project(&batch, projections).map_err(FalconError::Execution)?;
                    let out_rows = projected.to_rows();
                    if let Some(max) = early_limit {
                        result_rows.extend(out_rows.into_iter().take(max.saturating_sub(result_rows.len())));
                    } else {
                        result_rows.extend(out_rows);
                    }
                } else {
            ...
            if !has_window && raw_rows.len() >= 256 && is_vectorizable(projections, None) {
                // ── Vectorized project path (no filter, large batch) ──
                let num_cols = schema.columns.len();
                let batch = RecordBatch::from_row_pairs(&raw_rows, num_cols);
                let projected =
                    vectorized_project(&batch, projections).map_err(FalconError::Execution)?;
                let out_rows = projected.to_rows();
                if let Some(max) = early_limit {
                    result_rows.extend(out_rows.into_iter().take(max));
                } else {
                    result_rows.extend(out_rows);
                }
            } else {
    ...
    pub(crate) fn merge_rows_into(
        ...
        left: &OwnedRow,
        right: &OwnedRow,
        buf: &mut SmallVec<[Datum; 8]>,
    ) -> OwnedRow {
        buf.clear();
        buf.extend(left.values.iter().cloned());
        buf.extend(right.values.iter().cloned());
