...
/// Maximum number of plans to keep in the per-executor plan cache.
const PLAN_CACHE_MAX: usize = 1024;

/// Per-executor prepared statement plan cache.
/// Stores normalized SQL → Arc<PhysicalPlan>. Evicts oldest entry (FIFO) when full.
/// FIFO eviction avoids the per-access write overhead of LRU while still bounding memory.
pub(crate) struct PlanCache {
    inner: Mutex<IndexMap<String, Arc<PhysicalPlan>>>,
}

impl PlanCache {
    fn new() -> Self {
        Self {
            inner: Mutex::new(IndexMap::with_capacity(PLAN_CACHE_MAX)),
        }
    }

    /// Look up a plan by normalized SQL key. Returns a clone of the Arc (cheap).
    pub(crate) fn get(&self, key: &str) -> Option<Arc<PhysicalPlan>> {
        self.inner.lock().get(key).map(Arc::clone)
    }

    /// Insert a plan. Evicts the oldest entry if at capacity.
    pub(crate) fn insert(&self, key: String, plan: Arc<PhysicalPlan>) {
        let mut map = self.inner.lock();
        if map.len() >= PLAN_CACHE_MAX {
            map.shift_remove_index(0);
        }
        map.insert(key, plan);
    }

    /// Invalidate all cached plans for a given table (called on DDL).
    pub(crate) fn invalidate_all(&self) {
        self.inner.lock().clear();
    }
}

// ── P1-3: Generic (Parameterized) Plan Cache ─────────────────────────────────

/// Maximum entries in the generic plan cache.
const GENERIC_PLAN_CACHE_MAX: usize = 512;

/// A cached generic plan entry: the plan template plus usage statistics.
#[derive(Clone)]
pub struct GenericPlanEntry {
    /// The physical plan with `BoundExpr::Parameter` nodes intact.
    pub plan: Arc<PhysicalPlan>,
    /// Number of executions using this cached plan.
    pub use_count: u64,
    /// DDL epoch at the time this plan was cached.  When the epoch advances
    /// (DDL ran) the entry is invalidated.
    pub ddl_epoch: u64,
}

/// Generic plan cache: parameterized SQL template → plan.
///
/// # How it works
/// 1. The caller **normalizes** a SQL string by replacing all literal constant
///    values with `$1`, `$2`, … placeholders (e.g.
///    `SELECT * FROM t WHERE id = 42` → `SELECT * FROM t WHERE id = $1`).
///    The original literal values are collected into a `params` slice.
/// 2. The template string is used as the cache key.
/// 3. On cache hit the stored `Arc<PhysicalPlan>` is returned so the caller
///    can execute it via `execute_with_params`, passing the extracted literals.
/// 4. DDL epoch tracking ensures stale plans are evicted after schema changes.
///
/// # Why this beats the text-cache for OLTP
/// A high-frequency OLTP loop like `SELECT ... WHERE pk = ?` with thousands of
/// distinct pk values would fill the text-cache (one entry per value) and cause
/// constant eviction.  The generic cache stores **one** entry and reuses it
/// for all values, cutting parse+plan latency to near-zero.
pub struct GenericPlanCache {
    inner: Mutex<IndexMap<String, GenericPlanEntry>>,
}

impl GenericPlanCache {
    pub fn new() -> Self {
        Self {
            inner: Mutex::new(IndexMap::with_capacity(GENERIC_PLAN_CACHE_MAX)),
        }
    }

    /// Look up by parameterized template key.
    /// Returns `None` if not found or if the DDL epoch has advanced.
    pub fn get(&self, template: &str, current_ddl_epoch: u64) -> Option<Arc<PhysicalPlan>> {
        let mut map = self.inner.lock();
        if let Some(entry) = map.get_mut(template) {
            if entry.ddl_epoch != current_ddl_epoch {
                // Schema changed — evict stale entry
                map.shift_remove(template);
                return None;
            }
            entry.use_count += 1;
    ...
    /// Store a plan under a parameterized template key.
    pub fn insert(&self, template: String, plan: Arc<PhysicalPlan>, ddl_epoch: u64) {
        let mut map = self.inner.lock();
        if map.len() >= GENERIC_PLAN_CACHE_MAX {
            map.shift_remove_index(0);
        }
        map.insert(
            template,
    ...
    /// Invalidate all entries (called on any DDL).
    pub fn invalidate_all(&self) {
        self.inner.lock().clear();
    }

    /// Number of cached generic plans.
    pub fn len(&self) -> usize {
        self.inner.lock().len()
    }

    /// Snapshot of cache entries for `SHOW falcon.plan_cache`.
    pub fn snapshot(&self) -> Vec<(String, u64, u64)> {
        self.inner
            .lock()
            .iter()
            .map(|(k, v)| (k.clone(), v.use_count, v.ddl_epoch))
            .collect()
    }
}

impl Default for GenericPlanCache {
    fn default() -> Self {
        Self::new()
    }
}

/// Normalize a SQL string for generic plan caching by replacing all
/// integer / float / string literals with sequential `$N` placeholders.
///
/// Returns `(template, literals)` where `template` is the parameterized SQL
/// and `literals` is the extracted values in order.
///
/// # Limitations
/// - Only handles single-quoted string literals and bare numeric literals.
/// - Does not handle dollar-quoted strings or complex expressions.
/// - For production use, the SQL parser layer should perform normalization
///   using the AST.  This function is a lightweight fallback.
pub fn normalize_sql_for_cache(sql: &str) -> (String, Vec<String>) {
    ...
    while i < n {
        ...
        if bytes[i] == b'\'' {
            ...
            literals.push(sql[i..j].to_owned());
        ...
        if bytes[i].is_ascii_digit()
            || (bytes[i] == b'-'
                ...
                // only treat '-' as numeric if preceded by operator/space/paren
                && i > 0
                && matches!(bytes[i - 1], b'=' | b'<' | b'>' | b',' | b'(' | b' '))
        {
            ...
            literals.push(sql[start..i].to_owned());
            let idx = literals.len();
            out.push('$');
            out.push_str(&idx.to_string());
            continue;
        ...
        // Already a parameter placeholder — keep as-is
...
pub struct Executor {
    ...
    /// Maximum groups allowed in hash aggregation before spilling (0 = unlimited/no-spill).
    pub(crate) hash_agg_group_limit: usize,
    /// Spill directory for hash aggregation overflow (None = disable spill, hard-error on overflow).
    pub(crate) hash_agg_spill_dir: Option<std::path::PathBuf>,
    ...
    /// opt1: Per-executor plan cache — normalized SQL → Arc<PhysicalPlan>.
    /// Eliminates parse+bind+plan on repeated identical OLTP queries.
    pub(crate) plan_cache: PlanCache,
    /// P1-3: Generic (parameterized) plan cache — SQL template → Arc<PhysicalPlan>.
    /// Allows queries with different literal values to share one cached plan.
    pub generic_plan_cache: GenericPlanCache,
    /// opt2: Per-executor table handle cache — TableId → Arc<MemTable>.
    /// Skips engine_tables DashMap lookup on every DML/query operation.
    pub(crate) table_handle_cache: DashMap<TableId, Arc<falcon_storage::memtable::MemTable>>,
    /// P0-4: Per-session prepared statement cache.
...
impl Executor {
    ...
    pub fn set_spill_config(&mut self, config: &SpillConfig) {
        ...
        self.hash_agg_spill_dir = Some(if config.temp_dir.is_empty() {
            std::env::temp_dir().join("falcon_agg_spill")
        } else {
    ...
    /// opt1: Look up a cached plan by normalized SQL text.
    pub fn get_plan_cached(&self, sql: &str) -> Option<Arc<PhysicalPlan>> {
        self.plan_cache.get(sql)
    }

    /// opt1: Store a plan in the cache.
    pub fn cache_plan(&self, sql: String, plan: Arc<PhysicalPlan>) {
        self.plan_cache.insert(sql, plan);
    }

    /// opt1+opt2: Invalidate all caches (called on any DDL that modifies schema/tables).
    pub fn invalidate_caches(&self) {
        self.plan_cache.invalidate_all();
        self.generic_plan_cache.invalidate_all();
        self.table_handle_cache.clear();
    }

    /// P1-3: Look up a cached generic (parameterized) plan by SQL template.
    /// Returns the plan if found and the DDL epoch matches.
    pub fn get_generic_plan_cached(
        &self,
        template: &str,
    ) -> Option<Arc<PhysicalPlan>> {
        let epoch = self.storage.ddl_epoch();
        self.generic_plan_cache.get(template, epoch)
    }

    /// P1-3: Store a plan in the generic plan cache under a SQL template key.
    pub fn cache_generic_plan(&self, template: String, plan: Arc<PhysicalPlan>) {
        let epoch = self.storage.ddl_epoch();
        self.generic_plan_cache.insert(template, plan, epoch);
    }

    /// opt2: Get a cached MemTable handle for a table, falling through to storage on miss.
    #[allow(dead_code)]
    pub(crate) fn get_table_handle(
        &self,
        table_id: falcon_common::types::TableId,
    ) -> Option<Arc<falcon_storage::memtable::MemTable>> {
        if let Some(cached) = self.table_handle_cache.get(&table_id) {
            let handle: &Arc<falcon_storage::memtable::MemTable> = cached.value();
            return Some(Arc::clone(handle));
        }
        if let Some(handle) = self.storage.get_table(table_id) {
            self.table_handle_cache.insert(table_id, Arc::clone(&handle));
            return Some(handle);
        }
        None
    }
    ...
    /// P0-3 + P0-4: Execute a **pre-classified** OLTP fast-path statement.
    ///
    /// This variant is called by session/protocol layers that maintain a
    /// `PreparedStatement` cache storing both the `PhysicalPlan` and the
    /// `OltpKind` produced by [`OltpKind::classify`] at PREPARE time.
    ///
    /// # What is bypassed compared to `execute_with_params`
    /// - `reset_statement_ts()` syscall  (P0-4)
    /// - `Instant::now()` timing syscall  (P0-4)
    /// - AI feature extraction + `predict_cost` + `record_feedback`  (P0-4)
    /// - AIOps `record()` write  (P0-4)
    /// - `substitute_params_plan` full-tree clone  (P0-3)
    /// - General `execute_inner` match dispatch  (P0-3)
    ///
    /// # Fallback
    /// If the `OltpExecutor` detects that triggers have been added to the
    /// table since PREPARE time it returns `Err` with a special internal
    /// message.  The caller must re-classify and fall back to
    /// `execute_with_params` in that case.
    ///
    /// When `oltp_kind` is `None` this method is identical to calling
    /// `execute_with_params`.
    pub fn execute_with_params_fast(
        &self,
        plan: &PhysicalPlan,
        txn: Option<&TxnHandle>,
        params: &[Datum],
        oltp_kind: Option<crate::oltp_classifier::OltpKind>,
    ) -> Result<ExecutionResult, FalconError> {
        // ── P0-3: OLTP fast path ─────────────────────────────────────────────
        if let Some(kind) = oltp_kind {
            // Guard: read_only check (single atomic load — no syscall)
            if self.is_read_only() {
                return Err(FalconError::ReadOnly(
                    "cannot execute DML on a read-only replica".into(),
                ));
            }
            let txn_ref = txn.ok_or_else(|| {
                FalconError::Internal("OLTP fast path requires an active transaction".into())
            })?;
            // Guard: transaction read-only flag
            Self::reject_if_txn_read_only(txn_ref, "OLTP DML")?;
            // Guard: transaction timeout
            Self::check_txn_timeout(txn_ref)?;

            let exec = crate::oltp_executor::OltpExecutor {
                storage: &self.storage,
                txn_mgr: &self.txn_mgr,
            };
            // P0-4: no reset_statement_ts, no Instant::now, no AI telemetry
            let result = exec.dispatch(kind, plan, params, txn_ref);

            // If triggers appeared after PREPARE the fast-path executor returns
            // an Internal error with a specific prefix.  Re-run on general path.
            if let Err(FalconError::Internal(ref msg)) = result {
                if msg.starts_with("oltp fast path: triggers appeared after prepare") {
                    return self.execute_with_params(plan, txn, params);
                }
            }
            return result;
        }

        // ── General path (no hint available) ────────────────────────────────
        self.execute_with_params(plan, txn, params)
    }
    ...
    pub fn execute_with_sql(
        ...
        let result = self.execute_inner(plan, txn);
        // opt1+opt2: Flush caches on successful DDL so stale plans/handles are not reused.
        if let Ok(ExecutionResult::Ddl { .. }) = &result {
    ...
    fn execute_inner(
        ...
        match plan {
            ...
            PhysicalPlan::Analyze { table_name } => {
                ...
                let rows = vec![OwnedRow::new(vec![
                    Datum::Text(stats.table_name.into()),
            ...
            PhysicalPlan::ShowReplicationSlots => {
                ...
                let rows = slots.into_iter().map(|s| OwnedRow::new(vec![
                    Datum::Text(s.name.into()),
                    Datum::Text(s.plugin.into()),
            ...
            PhysicalPlan::ShowMemoryProfile => {
                ...
                let rows = vec![
                    ...
                    OwnedRow::new(vec![Datum::Text("pressure_state".into()), Datum::Text(snap.pressure_state.into())]),
    ...
    fn exec_call_procedure(
        ...
        let _result = match func_def.language {
            FunctionLanguage::Sql => {
                crate::plpgsql::execute_sql_function(&func_def, &arg_vals, eval_sql)
                    .map_err(FalconError::Execution)?
            }
