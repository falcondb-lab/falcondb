...
mod like_match_regression {
    ...
    fn like(s: &str, pattern: &str) -> Datum {
        let r = row(vec![Datum::Text(s.to_string())]);
        let expr = BoundExpr::Like {
            expr: Box::new(BoundExpr::ColumnRef(0)),
            pattern: Box::new(BoundExpr::Literal(Datum::Text(pattern.to_string()))),
            negated: false,
            case_insensitive: false,
        };
        eval_expr(&expr, &r).unwrap()
...
mod merge_rows_into_regression {
    ...
    fn buffer_reuse_across_calls() {
        ...
        let mut buf: smallvec::SmallVec<[Datum; 8]> = smallvec::SmallVec::new();
        ...
        buf.clear();
        buf.extend(left2.values.iter().cloned());
        buf.extend(right2.values.iter().cloned());
