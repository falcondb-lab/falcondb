...
impl QueryHandler {
        ...
        for t in tables {
            ...
            for (i, col) in t.columns.iter().enumerate() {
                ...
                rows.push(vec![
                    ...
                    None, // character_maximum_length (Cedar has no VARCHAR(n) yet)
    ...
    fn handle_information_schema_schemata(&self) -> Vec<BackendMessage> {
        self.single_row_result(
            ...
            vec![
                vec![Some("cedar".into()), Some("public".into()), Some("cedar".into())],
                vec![Some("cedar".into()), Some("information_schema".into()), Some("cedar".into())],
            ],
        )
    }
    ...
    fn handle_pg_settings(&self) -> Vec<BackendMessage> {
        ...
        let rows = vec![
            vec![Some("server_version".into()), Some("15.0".into()), None, Some("Version".into()), Some("CedarDB PG-compatible version".into())],
