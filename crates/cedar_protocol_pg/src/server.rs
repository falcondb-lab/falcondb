...
use cedar_common::config::{AuthConfig, AuthMethod};
use cedar_common::types::ShardId;
use cedar_executor::Executor;
use cedar_storage::engine::StorageEngine;
use cedar_txn::TxnManager;
...
impl PgServer {
    ...
    /// Used to share the counter between PgServer and Executor for SHOW cedar.connections.
    ...
    pub async fn run(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        tracing::info!("CedarDB PG server listening on {}", self.listen_addr);
        ...
        while let Some(msg) = codec::decode_message(&mut buf)? {
            match msg {
                ...
                FrontendMessage::Parse { name, query, param_types } => {
                    ...
                    let (plan, inferred_param_types, row_desc) = match handler.prepare_statement(&query) {
                        Ok((p, ipt, rd)) => {
                            ...
                            cedar_observability::record_prepared_stmt_parse_duration_us(dur, true);
                            cedar_observability::record_prepared_stmt_op("parse", "plan");
                            (Some(p), ipt, rd)
                        }
                        Err(_e) => {
                            let dur = parse_start.elapsed().as_micros() as u64;
                            cedar_observability::record_prepared_stmt_parse_duration_us(dur, false);
                            cedar_observability::record_prepared_stmt_op("parse", "legacy");
                ...
                FrontendMessage::Bind { portal, statement, param_formats, param_values, .. } => {
                    ...
                    cedar_observability::record_prepared_stmt_param_count(param_values.len());
                    let ps = session.prepared_statements.get(&statement);
                    let (plan, params_datum, bound_sql) = if let Some(ps) = ps {
                        let datum_params: Vec<cedar_common::datum::Datum> = param_values.iter().enumerate().map(|(i, pv)| {
                    ...
                    cedar_observability::record_prepared_stmt_bind_duration_us(bind_dur);
                    cedar_observability::record_prepared_stmt_op("bind", path);
                    cedar_observability::record_prepared_stmt_portals_active(session.portals.len());
                ...
                FrontendMessage::Describe { kind, name } => {
                    ...
                    cedar_observability::record_prepared_stmt_op("describe", if kind == b'S' { "statement" } else { "portal" });
                ...
                FrontendMessage::Execute { portal, .. } => {
                    ...
                    if let Some(p) = portal_data {
                                ...
                                tokio::task::spawn_blocking({
                                    ...
                                    move || {
                                        let responses = if let Some(ref plan) = p.plan {
                                            cedar_observability::record_prepared_stmt_op("execute", "plan");
                                            handler.execute_plan(plan, &p.params, &mut sess)
                                        } else if !p.bound_sql.is_empty() {
                                            cedar_observability::record_prepared_stmt_op("execute", "legacy");
                                            handler.handle_query(&p.bound_sql, &mut sess)
                                        } else {
                                            cedar_observability::record_prepared_stmt_op("execute", "empty");
                        ...
                        cedar_observability::record_prepared_stmt_execute_duration_us(exec_dur, success);
                        for response in &responses {
                            send_message(&mut stream, response).await?;
                        }
                    } else {
                ...
                FrontendMessage::Close { kind, name } => {
                    if kind == b'S' {
                        ...
                        cedar_observability::record_prepared_stmt_op("close", "statement");
                        cedar_observability::record_prepared_stmt_active(session.prepared_statements.len());
                    } else {
                        session.portals.remove(&name);
                        cedar_observability::record_prepared_stmt_op("close", "portal");
                        cedar_observability::record_prepared_stmt_portals_active(session.portals.len());
                    }
...
fn decode_param_value(
    raw: &Option<Vec<u8>>,
    type_hint: Option<&cedar_common::types::DataType>,
) -> cedar_common::datum::Datum {
...
mod tests {
    ...
    fn test_tls_config_disabled_by_default() {
        use cedar_common::config::TlsConfig;
    ...
    fn test_node_role_writable_classification() {
        use cedar_common::config::NodeRole;
    ...
    fn test_shutdown_drain_timeout_default() {
        use cedar_common::config::CedarConfig;
    ...
    fn test_shutdown_drain_timeout_configurable() {
        use cedar_common::config::ServerConfig;
        use cedar_common::config::{AuthConfig, TlsConfig};
