...
use cedar_cluster::{DistributedQueryEngine, ShardedEngine};
use cedar_common::config::{CedarConfig, NodeRole};
use cedar_common::types::ShardId;
use cedar_executor::Executor;
use cedar_protocol_pg::server::PgServer;
use cedar_storage::engine::StorageEngine;
use cedar_storage::memory::MemoryBudget;
use cedar_txn::TxnManager;

#[derive(Parser, Debug)]
#[command(name = "cedar", about = "CedarDB — PG-Compatible In-Memory OLTP")]
struct Cli {
    /// Config file path.
    #[arg(short, long, default_value = "cedar.toml")]
    config: String,
...
async fn main() -> Result<()> {
    ...
    cedar_observability::init_tracing();
    tracing::info!("Starting CedarDB...");
    ...
    // Expose node role to SHOW cedar.node_role via env var
    std::env::set_var("CEDAR_NODE_ROLE", format!("{:?}", config.replication.role).to_lowercase());

    // Initialize metrics
    if let Err(e) = cedar_observability::init_metrics(&cli.metrics_addr) {
    ...
    let replication_log = if config.replication.role == NodeRole::Primary {
        Some(Arc::new(cedar_cluster::ReplicationLog::new()))
    } else {
        None
    };
    ...
    let storage = if config.storage.wal_enabled {
        ...
        let mut engine = if data_dir.join("cedar.wal").exists() {
    ...
    if config.server.auth.method != cedar_common::config::AuthMethod::Trust {
    ...
    let mut replica_runner_handle: Option<cedar_cluster::ReplicaRunnerHandle> = None;
    match config.replication.role {
        NodeRole::Primary => {
            ...
            let svc = cedar_cluster::grpc_transport::WalReplicationService::new();
            ...
            tokio::spawn(async move {
                use cedar_cluster::proto::wal_replication_server::WalReplicationServer;
        ...
        NodeRole::Replica => {
            let runner_config = cedar_cluster::ReplicaRunnerConfig {
            ...
            let runner = cedar_cluster::ReplicaRunner::new(runner_config, storage.clone());
            let handle = runner.start();
            // Share metrics with PgServer for SHOW cedar.replica_stats
    ...
    let pg_port = config
        ...
        .unwrap_or("5433");
    tracing::info!(
        "CedarDB ready (role={:?}, {} shard{}). Connect with: psql -h 127.0.0.1 -p {}",
...
fn load_config(path: &str) -> CedarConfig {
