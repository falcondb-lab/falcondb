//! Cedar YCSB-style benchmark harness.
//!
...
//!
//! Usage:
//!   cargo run -p cedar_bench -- --ops 10000 --read-pct 50 --local-pct 80 --shards 4
//!   cargo run -p cedar_bench -- --export csv
...
use cedar_cluster::distributed_exec::{AggMerge, DistributedExecutor, GatherStrategy, SubPlan};
use cedar_cluster::sharded_engine::ShardedEngine;
...
use cedar_txn::manager::{SlowPathMode, TxnClassification, TxnManager};

/// Cedar YCSB-style benchmark.
#[derive(Parser, Debug)]
#[command(name = "cedar-bench", about = "YCSB-style benchmark for Cedar")]
struct Args {
...
struct BenchResult {
    ...
    elapsed_ms: u64,
    tps: f64,
    stats: cedar_txn::TxnStatsSnapshot,
}
...
fn run_failover_bench(args: &Args) {
    ...
    use cedar_common::types::{Timestamp, TxnId, TxnType};
