...
impl ChannelTransport {
    ...
    pub fn push_chunk(&self, chunk: WalChunk) -> Result<(), CedarError> {
        self.sender
            .lock()
            .send(chunk)
            .map_err(|e| CedarError::Internal(format!("Channel send error: {}", e)))
    }
...
impl ReplicationTransport for ChannelTransport {
    fn pull_wal_chunk(
        ...
        match rx.try_recv() {
            ...
            Err(e) => Err(CedarError::Internal(format!("Channel recv error: {}", e))),
        }
    }
