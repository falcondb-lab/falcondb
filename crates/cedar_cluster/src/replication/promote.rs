...
use cedar_common::types::{NodeId, ShardId, TxnId};
...
impl ShardReplicaGroup {
    ...
    pub fn new(shard_id: ShardId, schemas: &[TableSchema]) -> Result<Self, CedarError> {
    ...
    pub fn ship_wal_record(&self, record: cedar_storage::wal::WalRecord) -> u64 {
        self.log.append(record)
    }

    /// Apply pending WAL records to a specific replica via the replication log.
    /// Returns the number of records applied.
    pub fn catch_up_replica(&self, replica_idx: usize) -> Result<usize, CedarError> {
        let replica = self.replicas.get(replica_idx).ok_or_else(|| {
            CedarError::Internal(format!("Replica index {} out of range", replica_idx))
        })?;
    ...
    pub fn apply_chunk_to_replica(
        ...
        replica_idx: usize,
        chunk: &WalChunk,
    ) -> Result<usize, CedarError> {
        if !chunk.verify_checksum() {
            return Err(CedarError::Internal("WalChunk checksum mismatch".into()));
        }
        if chunk.is_empty() {
            return Ok(0);
        }
    ...
    pub fn promote(&mut self, replica_idx: usize) -> Result<(), CedarError> {
    ...
    fn promote_inner(&mut self, replica_idx: usize) -> Result<(), CedarError> {
