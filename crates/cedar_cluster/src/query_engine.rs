...
use cedar_common::types::{IsolationLevel, ShardId};
use cedar_executor::executor::{ExecutionResult, Executor};
use cedar_planner::plan::{DistAggMerge, DistGather, PhysicalPlan};
use cedar_sql_frontend::types::BoundExpr;
...
impl DistributedQueryEngine {
        ...
        match plan {
            ...
            PhysicalPlan::ExplainAnalyze(_) => {
                let shard = self.engine.shard(ShardId(0)).ok_or_else(|| {
                    CedarError::Internal("No shard 0 available".into())
                })?;
    ...
    pub fn aggregate_txn_stats(&self) -> cedar_txn::TxnStatsSnapshot {
        let mut agg = cedar_txn::TxnStatsSnapshot::default();
    ...
    /// Per-shard statistics for observability (SHOW cedar.shard_stats).
    ...
    /// Per-shard detailed load with per-table breakdown (SHOW cedar.shards).
    ...
    /// Get the rebalancer status (SHOW cedar.rebalance_status).
    ...
    fn exec_explain_coordinator_join(
        ...
        let (left_table_id, joins, filter, order_by, limit, offset) = match plan {
            ...
            _ => return Err(CedarError::Internal("exec_explain_coordinator_join: not a join plan".into())),
        };
        {
    ...
    fn extract_table_ids(plan: &PhysicalPlan) -> Vec<cedar_common::types::TableId> {
    ...
    fn collect_table_ids_from_plan(plan: &PhysicalPlan, ids: &mut std::collections::HashSet<cedar_common::types::TableId>) {
        match plan {
            PhysicalPlan::SeqScan { table_id, filter, projections, having, ctes, unions, .. } => {
                ...
                for proj in projections {
                    match proj {
                        cedar_sql_frontend::types::BoundProjection::Expr(e, _)
                        | cedar_sql_frontend::types::BoundProjection::Aggregate(_, Some(e), _, _, _) => {
    ...
    fn collect_table_ids_from_select(sel: &cedar_sql_frontend::types::BoundSelect, ids: &mut std::collections::HashSet<cedar_common::types::TableId>) {
    ...
    fn collect_table_ids_from_expr(expr: &BoundExpr, ids: &mut std::collections::HashSet<cedar_common::types::TableId>) {
        match expr {
    ...
    fn materialize_dml_filter(
        ...
        for &table_id in &ids {
            let schema = shard_ids.iter()
                ...
                .ok_or_else(|| CedarError::Internal(
                    format!("Table {:?} not found on any shard", table_id),
                ))?;

            temp_storage.create_table(schema.clone()).map_err(|e| {
                CedarError::Internal(format!("Failed to create temp table: {:?}", e))
            })?;

            let gathered_rows: Vec<cedar_common::datum::OwnedRow> = {
                let mut per_shard: Vec<Vec<cedar_common::datum::OwnedRow>> = Vec::new();
                std::thread::scope(|s| {
            ...
            for row in gathered_rows {
                temp_storage.insert(table_id, row, temp_txn.txn_id).map_err(|e| {
                    CedarError::Internal(format!("Failed to insert into temp storage: {:?}", e))
                })?;
            }
            temp_txn_mgr.commit(temp_txn.txn_id).map_err(|e| {
                CedarError::Internal(format!("Failed to commit temp txn: {:?}", e))
            })?;
        }
    ...
    fn exec_coordinator_join(
        ...
        let (left_table_id, joins) = match plan {
            ...
            _ => return Err(CedarError::Internal("exec_coordinator_join called on non-join plan".into())),
        };
        let mut table_ids = vec![left_table_id];
    ...
    fn gather_and_execute_locally(
        &self,
        table_ids: &[cedar_common::types::TableId],
        plan: &PhysicalPlan,
    ) -> Result<ExecutionResult, CedarError> {
        let temp_storage = Arc::new(StorageEngine::new_in_memory());
        let temp_txn_mgr = Arc::new(TxnManager::new(temp_storage.clone()));
    ...
    fn exec_ddl_all_shards(
        ...
        let results: Vec<Result<ExecutionResult, CedarError>> =
            std::thread::scope(|s| {
        ...
        first_result.ok_or_else(|| CedarError::Internal("No shards available".into()))
    }
    ...
    fn exec_insert_split(
        ...
        let (table_id, schema, columns, rows, source_select, returning, on_conflict) = match plan {
            ...
            _ => return Err(CedarError::Internal("exec_insert_split called with non-Insert".into())),
        };
        ...
        let results: Vec<(ShardId, Result<ExecutionResult, CedarError>)> =
            std::thread::scope(|s| {
                    ...
                    .map(|(shard_id, shard_plan)| {
                        ...
                        s.spawn(move || {
                            let shard = engine.shard(sid).ok_or_else(|| {
                                CedarError::Internal(format!("Shard {:?} not found", sid))
                            })?;
                            let local_exec =
    ...
    fn exec_insert_select(
        ...
        schema: &cedar_common::schema::TableSchema,
        columns: &[usize],
        source_select: &cedar_sql_frontend::types::BoundSelect,
        returning: &[(BoundExpr, String)],
        on_conflict: &Option<cedar_sql_frontend::types::OnConflictAction>,
    ) -> Result<ExecutionResult, CedarError> {
        // Plan the SELECT and execute at coordinator level to see all shard data
        let select_plan = cedar_planner::Planner::plan(
            &cedar_sql_frontend::types::BoundStatement::Select(source_select.clone()),
        )?;
    ...
    fn exec_gc_all_shards(&self) -> Result<ExecutionResult, CedarError> {
        let gc_results: Vec<Result<ExecutionResult, CedarError>> =
            std::thread::scope(|s| {
                let handles: Vec<_> = self.engine.all_shards()
                    .iter()
                    .map(|shard| {
    ...
    fn exec_dml_on_shards(
        ...
        let results: Vec<(u64, Result<ExecutionResult, CedarError>)> =
            std::thread::scope(|s| {
    ...
    fn merge_dml_results(
        results: Vec<(impl std::fmt::Display, Result<ExecutionResult, CedarError>)>,
        shard_count: usize,
    ) -> Result<ExecutionResult, CedarError> {
        let mut total_rows_affected = 0u64;
        let mut tag = String::new();
        ...
        let mut returning_columns: Option<Vec<(String, cedar_common::types::DataType)>> = None;
        let mut returning_rows: Vec<cedar_common::datum::OwnedRow> = Vec::new();
    ...
    fn exec_dml_all_shards(
        ...
        let results: Vec<(usize, Result<ExecutionResult, CedarError>)> =
            std::thread::scope(|s| {
                let handles: Vec<_> = self.engine.all_shards()
                    .iter()
                    .enumerate()
    ...
    fn resolve_filter_shard(
        &self,
        schema: &cedar_common::schema::TableSchema,
        filter: Option<&cedar_sql_frontend::types::BoundExpr>,
    ) -> Option<ShardId> {
    ...
    fn extract_pk_key(
        &self,
        schema: &cedar_common::schema::TableSchema,
        filter: &cedar_sql_frontend::types::BoundExpr,
    ) -> Option<i64> {
        use cedar_sql_frontend::types::{BinOp, BoundExpr};
    ...
    fn match_pk_eq(
        &self,
        pk_col_idx: usize,
        col_expr: &cedar_sql_frontend::types::BoundExpr,
        val_expr: &cedar_sql_frontend::types::BoundExpr,
    ) -> Option<i64> {
        use cedar_sql_frontend::types::BoundExpr;
        match col_expr {
    ...
    fn exec_dist_plan(
        ...
        for &sid in target_shards {
            if self.engine.shard(sid).is_none() {
                return Err(CedarError::Internal(format!("Shard {:?} not found", sid)));
            }
        }
        ...
        let shard_results: Vec<Result<ShardResult, CedarError>> =
            std::thread::scope(|s| {
                let handles: Vec<_> = target_shards
                    ...
                    .map(|&shard_id| {
                        ...
                        s.spawn(move || {
                            ...
                            Err(last_err.unwrap_or_else(|| CedarError::Internal(
        ...
        let merged_rows = match gather {
            ...
            DistGather::TwoPhaseAgg {
                ...
                if let Some(having_expr) = having {
                    merged.retain(|row| {
                        cedar_executor::expr_engine::ExprEngine::eval_filter(having_expr, row).unwrap_or(false)
                    });
                }
