...
impl RebalancePlanner {
    ...
    pub fn plan(&self, snapshot: &ShardLoadSnapshot, engine: &ShardedEngine) -> MigrationPlan {
        ...
        for _ in 0..max_iterations {
            ...
            *load_map.get_mut(&donor_id).unwrap() -= rows_to_move;
            *load_map.get_mut(&receiver_id).unwrap() += rows_to_move;
        }
...
impl ShardMigrator {
    ...
    pub fn execute_task(
        ...
        let mut batch_keys: Vec<(cedar_storage::memtable::PrimaryKey, cedar_common::datum::OwnedRow)> = Vec::new();
    ...
    fn flush_batch(
        &self,
        batch: &[(cedar_storage::memtable::PrimaryKey, cedar_common::datum::OwnedRow)],
...
impl RebalanceRunner {
    ...
    pub fn start(
        ...
        let join_handle = std::thread::Builder::new()
            .name("cedar-rebalancer".to_string())
            .spawn(move || {
...
fn datum_to_i64(d: &cedar_common::datum::Datum) -> Option<i64> {
...
mod tests {
    ...
    use cedar_storage::memtable::encode_pk_from_datums;
    ...
    fn test_imbalance_ratio_calculation() {
        ...
        // avg = 550, max = 1000, ratio = 1000/550 鈮?1.818
        let ratio = snapshot.imbalance_ratio();
