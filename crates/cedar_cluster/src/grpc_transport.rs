...
//!
//! Requires: `cargo build -p cedar_cluster --features grpc-codegen` to generate
//! the tonic client/server stubs from `proto/cedar_replication.proto`.
...
pub fn encode_wal_record(record: &LsnWalRecord) -> Result<Vec<u8>, CedarError> {
    serde_json::to_vec(record)
        .map_err(|e| CedarError::Internal(format!("WAL record serialization error: {}", e)))
}

/// Deserialize a `LsnWalRecord` from JSON bytes received in proto `record_payload`.
pub fn decode_wal_record(payload: &[u8]) -> Result<LsnWalRecord, CedarError> {
    serde_json::from_slice(payload)
        .map_err(|e| CedarError::Internal(format!("WAL record deserialization error: {}", e)))
}

/// Serialize a `WalChunk` to JSON bytes for network transport.
pub fn encode_wal_chunk(chunk: &WalChunk) -> Result<Vec<u8>, CedarError> {
    serde_json::to_vec(chunk)
        .map_err(|e| CedarError::Internal(format!("WalChunk serialization error: {}", e)))
}

/// Deserialize a `WalChunk` from JSON bytes received over the network.
pub fn decode_wal_chunk(payload: &[u8]) -> Result<WalChunk, CedarError> {
    let chunk: WalChunk = serde_json::from_slice(payload)
        .map_err(|e| CedarError::Internal(format!("WalChunk deserialization error: {}", e)))?;
    if !chunk.verify_checksum() {
        return Err(CedarError::Internal("WalChunk checksum mismatch after deserialization".into()));
    }
    Ok(chunk)
}
...
impl GrpcTransport {
    ...
    async fn get_channel(&self) -> Result<tonic::transport::Channel, CedarError> {
        let mut guard = self.channel.lock().await;
        if let Some(ref ch) = *guard {
            return Ok(ch.clone());
        }
        let ch = tonic::transport::Endpoint::from_shared(self.endpoint.clone())
            .map_err(|e| CedarError::Internal(format!("Invalid endpoint: {}", e)))?
            .connect_timeout(self.connect_timeout)
            .connect()
            .await
            .map_err(|e| CedarError::Internal(format!("gRPC connect error: {}", e)))?;
        *guard = Some(ch.clone());
        Ok(ch)
    }
...
impl GrpcTransport {
    ...
    pub async fn download_checkpoint(
        &self,
        shard_id: ShardId,
    ) -> Result<(cedar_storage::wal::CheckpointData, u64), CedarError> {
        ...
        let response = match client.get_checkpoint(request).await {
            ...
            Err(e) => {
                ...
                return Err(CedarError::Internal(format!("gRPC get_checkpoint error: {}", e)));
            }
        };
        ...
        let asm = assembler.ok_or_else(|| {
            CedarError::Internal("Checkpoint stream returned no chunks".into())
        })?;
    ...
    pub async fn subscribe_wal_stream(
        ...
        from_lsn: u64,
        max_records: usize,
    ) -> Result<tokio::sync::mpsc::Receiver<Result<WalChunk, CedarError>>, CedarError> {
        use crate::proto::wal_replication_client::WalReplicationClient;
        use tokio_stream::StreamExt;
        ...
        let response = match client.subscribe_wal(request).await {
            ...
            Err(e) => {
                self.invalidate_channel().await;
                return Err(CedarError::Internal(format!("gRPC subscribe_wal error: {}", e)));
            }
        };
        ...
        tokio::spawn(async move {
            loop {
                match stream.next().await {
                    ...
                    Some(Err(e)) => {
                        let _ = tx.send(Err(CedarError::Internal(
...
impl AsyncReplicationTransport for GrpcTransport {
    async fn pull_wal_chunk(
        ...
        match stream.next().await {
            ...
            Some(Err(e)) => {
                self.invalidate_channel().await;
                Err(CedarError::Internal(format!("gRPC stream error: {}", e)))
            }
    ...
    async fn ack_wal(
        ...
        if let Err(e) = client.ack_wal(request).await {
            self.invalidate_channel().await;
            return Err(CedarError::Internal(format!("gRPC ack_wal error: {}", e)));
        }
...
pub struct WalReplicationService {
    ...
    storage: parking_lot::RwLock<Option<Arc<cedar_storage::engine::StorageEngine>>>,
}
...
impl WalReplicationService {
    ...
    pub fn set_storage(&self, storage: Arc<cedar_storage::engine::StorageEngine>) {
    ...
    pub fn handle_pull(
        ...
        let log = self.logs.get(&shard_id).ok_or_else(|| {
            CedarError::Internal(format!("No replication log for shard {:?}", shard_id))
        })?;
    ...
    pub fn handle_ack(
        ...
        // monotonically correlated in CedarDB's single-node WAL model.
        if let Some(ref storage) = *self.storage.read() {
            storage
                .replica_ack_tracker()
                .update_ack(replica_id, cedar_common::types::Timestamp(applied_lsn));
        }
    }
...
pub fn wal_chunk_to_proto(chunk: &WalChunk) -> Result<crate::proto::WalChunkMessage, CedarError> {
    let records = chunk
        .records
        .iter()
        .map(|r| {
            let payload = serde_json::to_vec(r)
                .map_err(|e| CedarError::Internal(format!("Record serialization: {}", e)))?;
...
pub fn proto_to_wal_chunk(msg: &crate::proto::WalChunkMessage) -> Result<WalChunk, CedarError> {
    let records = msg
        .records
        .iter()
        .map(|entry| {
            serde_json::from_slice::<LsnWalRecord>(&entry.record_payload)
                .map_err(|e| CedarError::Internal(format!("Record deserialization: {}", e)))
        })
        .collect::<Result<Vec<_>, CedarError>>()?;
...
impl CheckpointStreamer {
    ...
    pub fn from_checkpoint_data(
        ckpt: &cedar_storage::wal::CheckpointData,
    ) -> Result<Self, CedarError> {
        let bytes = serde_json::to_vec(ckpt)
            .map_err(|e| CedarError::Internal(format!("Checkpoint serialization error: {}", e)))?;
...
impl CheckpointAssembler {
    ...
    pub fn add_chunk(&mut self, chunk: &CheckpointChunk) -> Result<bool, CedarError> {
    ...
    pub fn assemble(&self) -> Result<Vec<u8>, CedarError> {
        if !self.is_complete() {
    ...
    pub fn assemble_checkpoint(&self) -> Result<cedar_storage::wal::CheckpointData, CedarError> {
        let bytes = self.assemble()?;
        serde_json::from_slice(&bytes)
            .map_err(|e| CedarError::Internal(format!("Checkpoint deserialization error: {}", e)))
    }
