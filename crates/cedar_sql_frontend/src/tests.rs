...
mod binder_tests {
    ...
    fn bind_sql(sql: &str) -> Result<BoundStatement, cedar_common::error::SqlError> {
...
mod volatile_tests {
    ...
    fn test_expr_no_volatile() {
        ...
        let expr = BoundExpr::BinaryOp {
            ...
            right: Box::new(BoundExpr::Function {
                ...
                args: vec![BoundExpr::Literal(cedar_common::datum::Datum::Text("hello".into()))],
            }),
        };
        assert!(!expr_has_volatile(&expr));
    }
    ...
    fn test_literal_not_volatile() {
        let expr = BoundExpr::Literal(cedar_common::datum::Datum::Int32(42));
        assert!(!expr_has_volatile(&expr));
    }
    ...
    fn test_volatile_nested_in_case() {
        ...
        let expr = BoundExpr::Case {
            operand: None,
            conditions: vec![BoundExpr::Literal(cedar_common::datum::Datum::Boolean(true))],
            results: vec![BoundExpr::Function { func: ScalarFunc::Random, args: vec![] }],
            else_result: Some(Box::new(BoundExpr::Literal(cedar_common::datum::Datum::Int32(0)))),
        };
        assert!(expr_has_volatile(&expr));
    }
}
