...
pub enum LogicalPlan {
    ...
    /// P1-9: Broadcast join — small side is replicated to every shard.
    /// `broadcast_is_right` = true means `broadcast` maps to the right side of
    /// the join condition; false means it maps to the left.
    BroadcastJoin {
        left: Box<Self>,
        broadcast: Box<Self>,
        join_info: BoundJoin,
    ...
    /// P1-7: Partition-pruned scan wrapper — instructs the executor to skip
    /// any partition whose ID is NOT in `keep_partitions`.
    PartitionPruned {
        input: Box<Self>,
        /// Partition IDs that match the query predicate and must be scanned.
        keep_partitions: Vec<u32>,
    },
