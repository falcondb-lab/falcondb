...
fn make_row(id: i32, val: &str) -> OwnedRow {
    OwnedRow::new(vec![Datum::Int32(id), Datum::Text(val.to_string())])
}
...
fn test_wal4_wal5_replay_convergence_and_completeness() {
    ...
    let mut vals1: Vec<Vec<Datum>> = rows1.iter().map(|(_, r)| r.values.to_vec()).collect();
    let mut vals2: Vec<Vec<Datum>> = rows2.iter().map(|(_, r)| r.values.to_vec()).collect();
...
fn test_wal2_idempotent_replay() {
    ...
    for _ in 0..3 {
        ...
        let snapshot: Vec<(Vec<u8>, Vec<Datum>)> =
            rows.into_iter().map(|(pk, row)| (pk, row.values.into_vec())).collect();
