...
//! - Compression saves memory (target �?0% reduction for repetitive data)
...
// ─── Memory savings: �?0% for repetitive data ──────────────────────────
...
fn test_memory_savings_repetitive_data() {
    ...
    assert!(
        savings_pct >= 30.0,
        "Expected �?0% memory savings for repetitive data, got {:.1}%",
        savings_pct
    );
}
