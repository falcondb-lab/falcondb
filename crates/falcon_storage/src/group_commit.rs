...
pub struct GroupCommitSyncer {
    wal: Arc<WalWriter>,
    state: Arc<(Mutex<SyncerState>, Condvar)>, // parking_lot Mutex+Condvar
...
impl GroupCommitSyncer {
    ...
    pub fn new(wal: Arc<WalWriter>, config: GroupCommitConfig) -> Arc<Self> {
        let syncer = Arc::new(Self {
            ...
            state: Arc::new((
                ...
                Condvar::new(), // parking_lot Condvar
            )),
            config,
    ...
    pub fn start_syncer(self: &Arc<Self>) -> Result<std::thread::JoinHandle<()>, StorageError> {
        self.start_syncer_on_core(None)
    }

    /// Like `start_syncer` but pins the syncer thread to `core_id` (P1-3).
    /// Pass `None` to skip affinity binding (equivalent to `start_syncer`).
    pub fn start_syncer_on_core(
        self: &Arc<Self>,
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                // P1-3: Pin syncer to a dedicated core to reduce scheduling jitter
                // on the fsync critical path.
    ...
    pub fn append_and_wait(&self, record: &WalRecord) -> Result<u64, StorageError> {
        ...
        // Notify syncer and get our sequence number.
        let seq = {
        ...
        // FUA/fsync takes ~21-30µs; at high TPS most completions land within ~500 spins,
        // avoiding the kernel futex round-trip for the majority of commits.
        let mut spins = 0u32;
        loop {
        ...
        // Slow path: parking_lot Condvar wait (no PoisonError, ~10ns vs ~20ns)
        let flush_failed = {
            let (lock, cvar) = &*self.state;
            let mut state = lock.lock();
            ...
            while state.fsynced_lsn <= seq
                ...
                && !state.flush_failed
            {
                cvar.wait_for(&mut state, timeout);
            }
            state.flush_failed
        };
    ...
    pub fn notify_and_wait(&self, count: u64) -> Result<(), StorageError> {
        ...
        // Slow path: parking_lot Condvar wait
        let flush_failed = {
            let (lock, cvar) = &*self.state;
            let mut state = lock.lock();
            let timeout = Duration::from_micros(self.config.flush_interval_us * 10);
    ...
    pub fn shutdown(&self) {
        ...
        cvar.notify_all(); // parking_lot Condvar::notify_all
    }
        ...
        loop {
            ...
            let should_flush = {
                ...
                if state.pending_count == 0 {
                    ...
                    // parking_lot Condvar releases the lock while sleeping.
                    cvar.wait_for(&mut state, flush_interval);
                    ...
                    cvar.wait_for(&mut state, coalesce_window);
