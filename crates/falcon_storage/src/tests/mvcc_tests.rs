...
#[test]
fn test_fast_commit_ts_invalidated_on_update() {
    let chain = VersionChain::new();
    let txn1 = TxnId(1);
    let txn2 = TxnId(2);

    // INSERT at ts=5 — sets fast_commit_ts=5
    chain.prepend(txn1, Some(row(vec![Datum::Int32(100)])));
    assert_eq!(chain.commit_no_report(txn1, Timestamp(5)), 1);

    // UPDATE at ts=20 — must invalidate fast_commit_ts
    chain.prepend(txn2, Some(row(vec![Datum::Int32(200)])));
    assert_eq!(chain.commit_no_report(txn2, Timestamp(20)), 0);

    // Read at ts=10: between INSERT and UPDATE — must see original value
    let r = chain.read_committed(Timestamp(10));
    assert_eq!(r.unwrap().values[0], Datum::Int32(100));

    // read_for_txn at ts=10 with unrelated txn must also see original
    let r = chain.read_for_txn(TxnId(99), Timestamp(10));
    assert_eq!(r.unwrap().values[0], Datum::Int32(100));

    // Read at ts=25: after UPDATE — sees new value
    let r = chain.read_committed(Timestamp(25));
    assert_eq!(r.unwrap().values[0], Datum::Int32(200));

    // is_visible must be correct at intermediate ts
    assert!(chain.is_visible(TxnId(99), Timestamp(10)));
    assert!(chain.is_visible(TxnId(99), Timestamp(25)));
}

#[test]
fn test_fast_commit_ts_invalidated_on_delete() {
    let chain = VersionChain::new();
    let txn1 = TxnId(1);
    let txn2 = TxnId(2);
    ...
    // DELETE at ts=20
    chain.prepend(txn2, None);
    assert_eq!(chain.commit_no_report(txn2, Timestamp(20)), -1);

    // Read at ts=10: row should still be visible (before delete)
    let r = chain.read_committed(Timestamp(10));
    assert_eq!(r.unwrap().values[0], Datum::Int32(42));
    assert!(chain.is_visible(TxnId(99), Timestamp(10)));

    // Read at ts=25: row should NOT be visible (after delete)
    assert!(chain.read_committed(Timestamp(25)).is_none());
    assert!(!chain.is_visible(TxnId(99), Timestamp(25)));
}
