...
fn test_wal_observer_fires_on_all_operations() {
    ...
    assert_eq!(
        counter.load(Ordering::SeqCst),
        1,
        "insert deferred — no observer yet"
    );
    ...
    assert_eq!(
        counter.load(Ordering::SeqCst),
        6,
        "commit should flush deferred + fire"
    );
...
fn test_gc_runner_with_dynamic_replica_acks() {
    ...
    let mut runner =
        GcRunner::start(engine.clone(), provider.clone(), config.clone()).expect("spawn in test");
