...
thread_local! {
    ...
    /// Set to true for Read Committed transactions: read-set validation is never
    /// performed under RC, so accumulating read_set entries (each costing a
    /// `pk.to_vec()` heap allocation) is pure waste.
    static TXN_IS_READ_COMMITTED: std::cell::Cell<bool> = const { std::cell::Cell::new(false) };
}
...
/// Mark the current thread's active transaction as Read Committed.
/// Under RC, read-set validation is never performed, so `record_read` will
/// skip the `pk.to_vec()` allocation entirely.
/// Must be called after `begin()` and cleared (with `false`) after `commit`/`abort`.
pub fn set_txn_read_committed(rc: bool) {
    TXN_IS_READ_COMMITTED.with(|c| c.set(rc));
}
...
/// Combined per-transaction local state. Merges write-set, read-set, write-bytes
/// tracking, and deferred WAL buffer into a single DashMap entry, eliminating the
/// separate `txn_read_sets` DashMap (P0-4: saves 2 DashMap ops per txn).
#[derive(Default)]
pub(crate) struct TxnLocalState {
    /// ALLOC-4: inline storage for ≤4 write ops — covers all single-stmt autocommit txns.
    pub write_set: SmallVec<[TxnWriteOp; 4]>,
    /// ALLOC-5: inline storage for ≤8 read ops — covers typical point-read autocommit txns.
    pub read_set: SmallVec<[TxnReadOp; 8]>,
    pub write_bytes: u64,
    pub wal_buf: Vec<WalRecord>,
    pub wal_preserialized: Vec<u8>,
    pub wal_pre_count: usize,
    pub cached_table: Option<Arc<MemTable>>,
    /// P0-5: Read-set bloom filter for OCC conflict pre-screening.
    ///
    /// Every `record_read(table_id, pk)` call inserts the key's hash into this
    /// filter.  At `validate_read_set` time, each key in the concurrent
    /// committer's *write-set* is checked against this filter first:
    /// - Filter returns `false` → key is definitely NOT in our read-set →
    ///   skip the expensive `has_committed_write_after` check.
    /// - Filter returns `true`  → key *may* be in our read-set → do the
    ///   full per-key version-chain scan (false positives are safe).
    ///
    /// Sized for 256 elements at 1% FPR (≈ 2.5 KB) — covers OLTP transactions
    /// that read up to ~256 rows before committing.
    pub read_set_bloom: Option<Box<crate::bloom_filter::BloomFilter>>,
}

thread_local! {
    pub(crate) static TXN_LOCAL_CACHE: std::cell::RefCell<HashMap<(u64, TxnId), TxnLocalState>> = std::cell::RefCell::new(HashMap::new());
}
...
pub struct StorageEngine {
    ...
    /// Per-transaction local state (write-set + read-set + write-bytes + WAL buffer).
    /// read_set was merged here from the former txn_read_sets DashMap (P0-4).
    ...
    // ── Fuzzy Checkpoint (W2 Architecture Gap) ───────────────────────────────
    /// WAL LSN recorded at the last successful checkpoint.
    /// On recovery, WAL replay starts from this LSN instead of segment 0,
    /// reducing restart time from O(total_WAL) to O(WAL_since_checkpoint).
    pub(crate) checkpoint_lsn: AtomicU64,
    /// Interval in seconds between background fuzzy checkpoints (0 = disabled).
    pub(crate) auto_checkpoint_interval_secs: AtomicU64,
}
...
impl StorageEngine {
    ...
    /// Return the current DDL epoch (monotonically increasing on every DDL statement).
    /// Consumers (e.g. plan-cache invalidation) use this to detect schema changes.
    #[inline]
    pub fn ddl_epoch(&self) -> u64 {
        self.ddl_epoch.load(std::sync::atomic::Ordering::Acquire)
    }
    ...
    fn base() -> Self {
        Self {
            ...
            auto_checkpoint_interval_secs: AtomicU64::new(0),
        }
    }
    ...
    /// Append multiple write-set entries in one TXN_LOCAL_CACHE borrow (P2-2).
    /// Use this in batch_insert to avoid N separate borrow_mut() calls.
    pub(crate) fn record_writes_batch(&self, txn_id: TxnId, ops: Vec<TxnWriteOp>) {
        if ops.is_empty() {
            return;
        }
        let key = (self.engine_id, txn_id);
        TXN_LOCAL_CACHE.with(|cell| {
            let mut map = cell.borrow_mut();
            let state = map.entry(key).or_default();
            state.write_set.extend(ops);
        });
    }

    pub(crate) fn record_read(&self, txn_id: TxnId, table_id: TableId, pk: &[u8]) {
        // P0-A: RC transactions never need read-set validation — skip the
        // pk.to_vec() heap allocation entirely. Same fast-exit as autocommit.
        if SKIP_READ_TRACKING.with(|c| c.get()) || TXN_IS_READ_COMMITTED.with(|c| c.get()) {
            return;
        }
        // opt8: write read_set into TLS TXN_LOCAL_CACHE instead of txn_local DashMap,
        // eliminating a DashMap shard lock on every point-read hot path.
        let key = (self.engine_id, txn_id);
        TXN_LOCAL_CACHE.with(|cell| {
            let mut map = cell.borrow_mut();
            let state = map.entry(key).or_default();
            // P0-5: Also insert into the read-set bloom filter for O(1) conflict
            // pre-screening at validate_read_set time.  Lazy-initialise the filter
            // on the first read so zero-read transactions pay no allocation cost.
            let bloom_key = Self::make_bloom_key(table_id, pk);
            let bloom = state.read_set_bloom.get_or_insert_with(|| {
                Box::new(crate::bloom_filter::BloomFilter::with_capacity_and_fpr(
                    256,
                    0.01,
                ))
            });
            bloom.insert(&bloom_key);
            state.read_set.push(TxnReadOp { table_id, pk: pk.to_vec() });
        });
    }

    /// Build the bloom filter key for a (table_id, pk) pair.
    /// Uses a fixed-size 12-byte encoding: 8 bytes table_id || first 4 bytes pk.
    #[inline]
    fn make_bloom_key(table_id: TableId, pk: &[u8]) -> [u8; 12] {
        let mut k = [0u8; 12];
        k[..8].copy_from_slice(&table_id.0.to_le_bytes());
        let pk_len = pk.len().min(4);
        k[8..8 + pk_len].copy_from_slice(&pk[..pk_len]);
        k
    }

    pub(crate) fn take_read_set(&self, txn_id: TxnId) -> Vec<TxnReadOp> {
        // opt8: read_set lives in TLS (written by record_read). Drain it from there first,
        // then also drain any overflow in the DashMap (e.g. from cross-thread flush).
        let key = (self.engine_id, txn_id);
        let mut tls_reads: Vec<TxnReadOp> = TXN_LOCAL_CACHE.with(|cell| {
            cell.borrow_mut()
                .get_mut(&key)
                .map_or_else(Vec::new, |s| std::mem::take(&mut s.read_set).into_vec())
        });
        if let Some(mut state) = self.txn_local.get_mut(&txn_id) {
            if !state.read_set.is_empty() {
                tls_reads.extend(std::mem::take(&mut state.read_set));
            }
        }
        tls_reads
    }

    /// Flush thread-local cache to global DashMap.
    /// Call this when inserts run on a spawned thread but commit will happen on a different thread.
    pub fn flush_local_cache(&self) {
        let eid = self.engine_id;
        TXN_LOCAL_CACHE.with(|cell| {
            let mut map = cell.borrow_mut();
            let keys: Vec<TxnId> = map.keys()
                .filter(|(e, _)| *e == eid)
                .map(|(_, id)| *id)
                .collect();
            for txn_id in keys {
                if let Some(state) = map.remove(&(eid, txn_id)) {
                    ...
                    // opt8: also flush read_set accumulated in TLS.
                    entry.read_set.extend(state.read_set);
    ...
    pub(crate) fn take_local_state(&self, txn_id: TxnId) -> TxnLocalState {
        let key = (self.engine_id, txn_id);
        let tl = TXN_LOCAL_CACHE.with(|cell| cell.borrow_mut().remove(&key));
        ...
        match (tl, dm) {
            ...
            (Some(mut a), Some(b)) => {
                ...
                // opt8: merge read_set from both halves.
                a.read_set.extend(b.read_set);
    ...
    pub fn write_set_snapshot(&self, txn_id: TxnId) -> usize {
        let key = (self.engine_id, txn_id);
        let tl_len = TXN_LOCAL_CACHE.with(|cell| {
            cell.borrow().get(&key).map_or(0, |s| s.write_set.len())
        });
        let dm_len = self.txn_local.get(&txn_id).map_or(0, |s| s.write_set.len());
        tl_len + dm_len
    }
    ...
    pub fn rollback_write_set_after(&self, txn_id: TxnId, snapshot_len: usize) {
        // Flush thread-local to DashMap first so we have a single mutable view.
    ...
    pub fn rollback_read_set_after(&self, txn_id: TxnId, snapshot_len: usize) {
        // P0-4: Check TLS first — avoids DashMap shard lock on savepoint hot path.
        let key = (self.engine_id, txn_id);
        let tls_truncated = TXN_LOCAL_CACHE.with(|cell| {
            let mut map = cell.borrow_mut();
            if let Some(s) = map.get_mut(&key) {
                s.read_set.truncate(snapshot_len);
                return true;
            }
            false
        });
        if !tls_truncated {
            if let Some(mut entry) = self.txn_local.get_mut(&txn_id) {
                entry.read_set.truncate(snapshot_len);
            }
        }
    }
    ...
    pub fn write_set_keys(&self, txn_id: TxnId) -> Vec<(u64, Vec<u8>)> {
        let key = (self.engine_id, txn_id);
        let mut result: Vec<(u64, Vec<u8>)> = TXN_LOCAL_CACHE.with(|cell| {
            cell.borrow().get(&key).map_or_else(Vec::new, |s| {
                s.write_set.iter().map(|op| (op.table_id.0, op.pk.clone())).collect()
            })
        });
        if let Some(s) = self.txn_local.get(&txn_id) {
            result.extend(s.write_set.iter().map(|op| (op.table_id.0, op.pk.clone())));
        }
        result
    }
    ...
    pub fn read_set_snapshot(&self, txn_id: TxnId) -> usize {
        // P0-4: TLS-first lookup — avoids DashMap shard lock on savepoint hot path.
        let key = (self.engine_id, txn_id);
        let tls_len = TXN_LOCAL_CACHE.with(|cell| {
            cell.borrow().get(&key).map_or(0, |s| s.read_set.len())
        });
        let dm_len = self.txn_local.get(&txn_id).map_or(0, |s| s.read_set.len());
        tls_len + dm_len
    }
                            ...
                            match delta {
                                1 => {
                                    table
                                        .committed_row_count
                                        .fetch_add(1, AtomicOrdering::Relaxed);
                                    table.pk_order_dirty.store(true, AtomicOrdering::Release);
                                }
                                -1 => {
                                    table
                                        .committed_row_count
    ...
    pub fn analyze_table(&self, table_name: &str) -> Result<TableStatistics, StorageError> {
        ...
        let rows: Vec<Vec<falcon_common::datum::Datum>> = handle
            ...
            .into_iter()
            .map(|(_pk, row)| row.values.into_vec())
            .collect();
    ...
    fn track_rows_modified(&self, write_set: &[TxnWriteOp]) {
        ...
        for op in write_set {
            let table = match self.engine_tables.get(&op.table_id)
                .and_then(|h| h.as_rowstore().cloned())
            {
                Some(t) => t,
                None => continue,
            ...
            if prev + 1 >= threshold {
                table.rows_modified.store_all_zero(AtomicOrdering::Relaxed);
                ...
                // P2-1: Route through the existing JobScheduler instead of spawning a raw
                // OS thread on the commit hot path. JobScheduler uses a fixed-size thread
                // pool — no per-commit thread creation (~10µs saved per trigger).
                let engine_ref = Arc::clone(&engine);
                engine.ext.job_scheduler.register(
                    "auto-analyze",
                    crate::job_scheduler::JobInterval::Once,
                    Box::new(move || {
                        let table_name = engine_ref
                            .catalog
                            .read()
                            .find_table_by_id(table_id)
                            .map(|s| s.name.clone());
                        if let Some(name) = table_name {
                            if let Ok(stats) = engine_ref.analyze_table(&name) {
                                tracing::debug!(
                                    table = %name,
    ...
    ///
    /// # P0-5: Bloom-filter pre-screen
    /// Before scanning the read-set for a given `op`, we check the read-set's
    /// bloom filter.  If the filter says the key is definitely absent, we skip
    /// the expensive `has_committed_write_after` version-chain scan.
    /// This is especially effective for large write-sets with mostly disjoint
    /// keys relative to this transaction's read-set.
    pub fn validate_read_set(
        &self,
        txn_id: TxnId,
        start_ts: Timestamp,
    ) -> Result<(), StorageError> {
        // Collect the bloom filter from TLS (if any) for pre-screening.
        // We take a snapshot of the filter pointer — the filter is read-only here.
        let key = (self.engine_id, txn_id);

        let check = |op: &TxnReadOp| -> bool {
            if let Some(handle) = self.engine_tables.get(&op.table_id) {
                if let Some(table) = handle.as_rowstore() {
                    return table.has_committed_write_after(&op.pk, txn_id, start_ts);
                }
            }
            false
        };

        // P0-5: bloom-pre-screen helper — returns true if the key passes the
        // filter (possible conflict) or if no filter exists (conservative).
        let bloom_may_conflict = |table_id: TableId, pk: &[u8]| -> bool {
            let bkey = Self::make_bloom_key(table_id, pk);
            TXN_LOCAL_CACHE.with(|cell| {
                cell.borrow()
                    .get(&key)
                    .and_then(|s| s.read_set_bloom.as_ref())
                    .map_or(true, |bf| bf.contains(&bkey))
            })
        };

        // Check TLS-resident read-set (written by record_read on the same thread).
        let tls_conflict = TXN_LOCAL_CACHE.with(|cell| {
            cell.borrow()
                .get(&key)
                .map_or(false, |s| {
                    s.read_set.iter().any(|op| {
                        // P0-5: skip full check if bloom says definitely absent.
                        if let Some(ref bf) = s.read_set_bloom {
                            let bkey = Self::make_bloom_key(op.table_id, &op.pk);
                            if !bf.contains(&bkey) {
                                return false; // definite non-conflict
                            }
                        }
                        check(op)
                    })
        ...
        // Also check DashMap (populated by flush_local_cache / cross-thread txns).
        if let Some(state) = self.txn_local.get(&txn_id) {
            for op in &state.read_set {
                // P0-5: bloom pre-screen before expensive version-chain scan.
                if !bloom_may_conflict(op.table_id, &op.pk) {
                    continue;
                }
                if check(op) {
                    return Err(StorageError::SerializationFailure);
                ...
                if prev + 1 >= threshold {
                        ...
                        .is_ok()
                    {
                        ...
                        if let Some(engine) = self.self_weak.read().upgrade() {
                            // P2-1: Use JobScheduler instead of raw thread spawn to avoid
                            // OS thread creation cost (~10µs) on the commit hot path.
                            let engine_ref = Arc::clone(&engine);
                            engine.ext.job_scheduler.register(
                                "auto-analyze",
                                crate::job_scheduler::JobInterval::Once,
                                Box::new(move || {
                                    if let Some(name) = engine_ref.catalog.read().find_table_by_id(tid).map(|s| s.name.clone()) {
                                        let _ = engine_ref.analyze_table(&name);
                                    }
                                    engine_ref.auto_analyze_running.store(false, AtomicOrdering::Release);
                                    Ok(())
                                }),
                            );
                        } else {
                            self.auto_analyze_running.store(false, AtomicOrdering::Release);
                        }
                    }
                }
                return Ok(());
    ...
    /// Estimate total bytes for a write-set.
    /// Returns 0 when `write_bytes` was not set by the DML path — the alloc/dealloc
    /// must be symmetric, so returning 0 is a safe no-op rather than under/over-counting.
    fn estimate_write_set_bytes(&self, _txn_id: TxnId, _write_set: &[TxnWriteOp]) -> u64 {
        0
    }
    ...
    pub fn run_gc_with_config(
        ...
        // VersionChain2 uses a per-table arena allocator; reclaimed nodes return
        // to the arena free-list rather than the global heap.  The mvcc_bytes
        // accounting is therefore not drained here — reclaimed_bytes is for
        // observability only (logged by the GC runner above).
    ...
    /// Return the WAL LSN of the last successful checkpoint.
    /// WAL recovery uses this to skip segments older than this LSN.
    #[inline]
    pub fn checkpoint_lsn(&self) -> u64 {
        self.checkpoint_lsn.load(std::sync::atomic::Ordering::Acquire)
    }

    /// Configure and launch a background thread that runs `checkpoint()` every
    /// `interval_secs` seconds.  Pass 0 to disable.
    ///
    /// The background thread holds a `Weak<StorageEngine>` so it stops
    /// automatically when the engine is dropped.
    pub fn start_auto_checkpoint(self: &Arc<Self>, interval_secs: u64) {
        self.auto_checkpoint_interval_secs
        ...
        std::thread::Builder::new()
            .name("falcon-auto-checkpoint".into())
            .spawn(move || {
                ...
                loop {
                    ...
                    match engine.fuzzy_checkpoint() {
                        Ok((seg, rows)) => {
                            tracing::debug!(
                                "auto-checkpoint: seg={seg}, rows={rows}, lsn={}",
                                engine.checkpoint_lsn()
                            );
                        }
                        Err(e) => {
            ...
            .expect("failed to spawn auto-checkpoint thread");
    }

    /// Fuzzy Checkpoint — non-blocking variant of `checkpoint()`.
    ///
    /// ## How it differs from the blocking checkpoint
    ///
    /// | | Blocking checkpoint | Fuzzy checkpoint |
    /// |---|---|---|
    /// | Scan | Holds no explicit lock but blocks WAL flush | Snapshot LSN first, scan concurrently |
    /// | WAL purge | Immediate | Deferred until snapshot is flushed |
    /// | Recovery | Replay from checkpoint segment | Replay from `checkpoint_lsn` only |
    ///
    /// ## Recovery shortcut
    /// After a successful fuzzy checkpoint, WAL recovery reads `checkpoint_lsn`
    /// from `checkpoint.bin` and **skips all WAL records with lsn < checkpoint_lsn**.
    /// This reduces restart time from O(all WAL) to O(WAL since last checkpoint).
    pub fn fuzzy_checkpoint(&self) -> Result<(u64, usize), StorageError> {
        let wal = self
            .wal
            .as_ref()
            .ok_or_else(|| StorageError::Serialization("fuzzy_checkpoint: no WAL".into()))?;

        // Phase 1: record the current LSN *before* scanning.
        // Any WAL records with lsn > snap_lsn written during the scan will be
        // replayed on the next recovery — they are not missing from the snapshot.
        let snap_lsn = wal.current_lsn();
        let snap_segment = wal.current_segment_id();

        // Phase 2: scan all committed rows (no exclusive lock required).
        let read_ts = Timestamp(u64::MAX - 1);
        let dummy_txn = TxnId(0);
        let catalog = self.catalog.read().clone();
        let mut table_data = Vec::new();
        ...
        for entry in &self.engine_tables {
            ...
            let rows: Vec<(Vec<u8>, OwnedRow)> = memtable.scan(dummy_txn, read_ts);
            total_rows += rows.len();
            table_data.push((*entry.key(), rows));
        }

        // Phase 3: write snapshot to disk.
        let ckpt = CheckpointData {
            catalog,
            table_data,
            wal_segment_id: snap_segment,
            wal_lsn: snap_lsn,
        };
        let wal_dir = wal.wal_dir();
        ckpt.write_to_dir(&wal_dir).map_err(|e| {
            StorageError::Wal(format!("fuzzy_checkpoint: write failed: {e}"))
        })?;

        // Phase 4: record checkpoint LSN in the WAL and flush.
        wal.append(&WalRecord::Checkpoint {
            timestamp: Timestamp(snap_lsn),
        })
        .map_err(|e| StorageError::Wal(format!("fuzzy_checkpoint: append failed: {e}")))?;
        wal.flush()
            .map_err(|e| StorageError::Wal(format!("fuzzy_checkpoint: flush failed: {e}")))?;

        // Phase 5: update in-memory checkpoint_lsn so recovery knows the start point.
        self.checkpoint_lsn
            .fetch_max(snap_lsn, std::sync::atomic::Ordering::Release);

        // Phase 6: purge WAL segments that are entirely before snap_segment.
        let checkpoint_path = wal_dir.join("checkpoint.bin");
        if checkpoint_path.exists() {
            let _ = wal.purge_segments_before(snap_segment);
        }

        tracing::info!(
            "Fuzzy checkpoint: lsn={snap_lsn}, segment={snap_segment}, rows={total_rows}"
        );
        Ok((snap_segment, total_rows))
    }
    ...
    pub fn apply_checkpoint_data(&self, ckpt: &CheckpointData) -> Result<(), StorageError> {
        ...
        for (table_id, rows) in &ckpt.table_data {
            ...
            if let Some(schema) = schema {
                ...
                let sentinel_ts = Timestamp(1);
                for (pk, row) in rows {
                    let chain = Arc::new(crate::version_chain2::VersionChain2::new(
                        Arc::as_ptr(&memtable.arena),
                    ));
                    chain.prepend(sentinel_txn, Some(Arc::new(row.clone())));
                    chain.commit_no_report(sentinel_txn, sentinel_ts);
                ...
                WalRecord::DropIndex {
                    ...
                    if let Some((_, meta)) = engine.index_registry.remove(index_name.as_str()) {
                        if let Some(handle) = engine.engine_tables.get(&meta.table_id) {
                            if let Some(memtable) = handle.as_rowstore() {
                                memtable.remove_secondary_index_by_column(*column_idx);
                            }
                        }
                    }
                }
