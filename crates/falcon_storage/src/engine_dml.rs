...
impl StorageEngine {
    ...
    /// Short-circuits immediately when CDC is disabled to avoid catalog RwLock acquire.
    #[inline]
...
impl StorageEngine {
    ...
    fn insert_rowstore(
        ...
        // P0-3: take ownership of pk; clone only once for the return value.
        ...
        // Clone pk once here for return; move original into write-set (no second clone).
        let pk_ret = pk.clone();
        crate::engine::TXN_LOCAL_CACHE.with(|cell| {
            ...
            if let Some(pre) = pre_bytes {
                state.wal_preserialized.extend(pre);
                state.wal_pre_count += pre_count;
            }
            // pk moved here — no additional clone needed.
    ...
    fn batch_insert_rowstore_inner(
        &self,
        table: &Arc<MemTable>,
        table_id: TableId,
        rows: Vec<OwnedRow>,
        ...
        // P1-2: Pre-serialize the BatchInsert WAL record once.
        let pre_bytes = if self.wal_observer.is_none() {
            crate::wal::WalWriter::serialize_record(&wal_record).ok()
        } else {
            None
        };
        let pre_count = usize::from(pre_bytes.is_some());
        ...
        let mut write_ops = Vec::with_capacity(row_count);
        for row in rows {
            let cdc_row = if cdc_enabled { Some(row.clone()) } else { None };
            let pk = table.insert(row, txn_id)?;
            write_ops.push(crate::engine::TxnWriteOp { table_id, pk: pk.clone() });
        ...
        // P1-2: Flush write_bytes + pre-serialized WAL + write ops to TXN_LOCAL_CACHE.
        let key = (self.engine_id, txn_id);
        crate::engine::TXN_LOCAL_CACHE.with(|cell| {
            let mut map = cell.borrow_mut();
            let state = map.entry(key).or_default();
            state.write_bytes += total_bytes;
            if let Some(pre) = pre_bytes {
                state.wal_preserialized.extend(pre);
                state.wal_pre_count += pre_count;
            }
        });
        self.record_writes_batch(txn_id, write_ops);
        Ok(pks)
    }
    ...
    pub fn batch_insert(
        ...
        // N-17: TABLE_CACHE TLS fast-path for rowstore (same pattern as insert()).
        let eid = self.engine_id;
        let epoch = self.ddl_epoch.load(std::sync::atomic::Ordering::Relaxed);
        let cached = TABLE_CACHE.with(|c| {
            let slot = c.borrow();
        ...
        if let Some(table) = cached {
            return self.batch_insert_rowstore_inner(&table, table_id, rows, txn_id);
        }
        ...
        match handle.value() {
            TableHandle::Rowstore(table) => {
                TABLE_CACHE.with(|c| *c.borrow_mut() = Some((eid, epoch, table_id, Arc::clone(table))));
                self.batch_insert_rowstore_inner(table, table_id, rows, txn_id)
            }
            #[cfg(feature = "columnstore")]
            TableHandle::Columnstore(cs) => {
                self.record_write_path_violation_columnstore()?;
                ...
                for row in &rows {
                    // N-08: encode_pk_stack avoids heap alloc for PKs ≤16 bytes.
                    let pk_buf = crate::memtable::encode_pk_stack(row, cs.schema.pk_indices());
                    pks.push(pk_buf.into_vec());
                }
