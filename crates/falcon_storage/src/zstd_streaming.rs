...
use crate::zstd_segment::{ZstdBlock, decompress_block};
...
pub fn negotiate_streaming_codec(sender: &StreamingCodecCaps, receiver: &StreamingCodecCaps) -> SegmentCodec {
    let common: Vec<SegmentCodec> = sender.supported.iter()
        .filter(|c| receiver.supported.contains(c))
        .copied().collect();
    if common.is_empty() { return SegmentCodec::None; }
    if (sender.preferred == SegmentCodec::Zstd || receiver.preferred == SegmentCodec::Zstd)
        && common.contains(&SegmentCodec::Zstd) { return SegmentCodec::Zstd; }
...
pub fn compress_streaming_chunk(data: &[u8], codec: SegmentCodec, level: i32) -> Result<Vec<u8>, String> {
    match codec {
        ...
        SegmentCodec::Zstd => zstd::bulk::compress(data, level).map_err(|e| format!("zstd stream compress: {e}")),
    }
}
...
pub fn decompress_streaming_chunk(data: &[u8], codec: SegmentCodec, max_size: usize) -> Result<Vec<u8>, String> {
    match codec {
        SegmentCodec::None => Ok(data.to_vec()),
        SegmentCodec::Lz4 => lz4_flex::decompress_size_prepended(data).map_err(|e| format!("lz4 stream decompress: {e}")),
        SegmentCodec::Zstd => zstd::bulk::decompress(data, max_size).map_err(|e| format!("zstd stream decompress: {e}")),
    }
}
...
impl DecompressCache {
    ...
    pub fn insert(&self, key: DecompressCacheKey, data: Vec<u8>) {
        ...
        while self.used_bytes.load(Ordering::Relaxed) + data_len > self.capacity_bytes {
            let evict_key = { let mut order = self.order.lock(); order.pop_front() };
...
impl DecompressPool {
    ...
    pub fn decompress(&self, segment_id: u64, block_index: u32, block: &ZstdBlock, dict: Option<&[u8]>) -> Result<Vec<u8>, String> {
        let cache_key = DecompressCacheKey { segment_id, block_index };
        if let Some(cached) = self.cache.get(&cache_key) { return Ok(cached); }
