...
pub fn encode_message_into(buf: &mut BytesMut, msg: &BackendMessage) {
    match msg {
        ...
        BackendMessage::AuthenticationSASL { mechanisms } => {
            ...
            // Compute body length upfront to write directly (avoids temp BytesMut + copy).
            let mechs_len: usize = mechanisms.iter().map(|m| m.len() + 1).sum::<usize>() + 1;
            let body_len = 4 + mechs_len; // i32 auth-type + cstrings + empty terminator
        ...
        BackendMessage::RowDescription { fields } => {
            // Compute body length upfront to write directly (avoids temp BytesMut + copy).
            // Each field: cstring(name) + i32 + i16 + i32 + i16 + i32 + i16 = name+1+18 bytes.
            let body_len: usize = 2 + fields.iter().map(|f| f.name.len() + 1 + 18).sum::<usize>();
            buf.reserve(1 + 4 + body_len);
            buf.put_u8(b'T');
            buf.put_i32(4 + body_len as i32);
        ...
        BackendMessage::ErrorResponse {
            ...
            // Compute body length upfront to write directly (avoids temp BytesMut + copy).
            // Fields: S + cstr(sev) + V + cstr(sev) + C + cstr(code) + M + cstr(msg) + NUL
            ...
            buf.put_u8(b'E');
            buf.put_i32(4 + body_len as i32);
            buf.put_u8(b'S'); write_cstring(buf, severity);
            buf.put_u8(b'V'); write_cstring(buf, severity);
            buf.put_u8(b'C'); write_cstring(buf, code);
            buf.put_u8(b'M'); write_cstring(buf, message);
            buf.put_u8(0);
        }
        BackendMessage::ErrorResponseExt {
            ...
            // Compute body length upfront to write directly (avoids temp BytesMut + copy).
            let opt_field_len = |s: &Option<String>| s.as_ref().map_or(0, |v| 1 + v.len() + 1);
            let pos_str; // storage for position integer formatting
            let pos_len = if let Some(p) = ef.position {
                pos_str = p.to_string();
                1 + pos_str.len() + 1
            } else {
                pos_str = String::new();
            ...
            let body_len = 1 + severity.len() + 1
                ...
                + 1 + code.len() + 1
                + 1 + message.len() + 1
                + opt_field_len(&ef.detail)
                + opt_field_len(&ef.hint)
                + pos_len
                + opt_field_len(&ef.schema_name)
                + opt_field_len(&ef.table_name)
                + opt_field_len(&ef.column_name)
                + opt_field_len(&ef.constraint_name)
                + opt_field_len(&ef.where_)
                + 1; // final NUL
            buf.reserve(1 + 4 + body_len);
            buf.put_u8(b'E');
            buf.put_i32(4 + body_len as i32);
            ...
            buf.put_u8(b'C'); write_cstring(buf, code);
            buf.put_u8(b'M'); write_cstring(buf, message);
            if let Some(ref d) = ef.detail   { buf.put_u8(b'D'); write_cstring(buf, d); }
            if let Some(ref h) = ef.hint     { buf.put_u8(b'H'); write_cstring(buf, h); }
            if ef.position.is_some()         { buf.put_u8(b'P'); write_cstring(buf, &pos_str); }
            if let Some(ref s) = ef.schema_name    { buf.put_u8(b's'); write_cstring(buf, s); }
            if let Some(ref t) = ef.table_name     { buf.put_u8(b't'); write_cstring(buf, t); }
            if let Some(ref c) = ef.column_name    { buf.put_u8(b'c'); write_cstring(buf, c); }
            if let Some(ref n) = ef.constraint_name{ buf.put_u8(b'n'); write_cstring(buf, n); }
            if let Some(ref w) = ef.where_         { buf.put_u8(b'W'); write_cstring(buf, w); }
            buf.put_u8(0);
        }
        BackendMessage::NoticeResponse { message } => {
            // Compute body length upfront to write directly (avoids temp BytesMut + copy).
            // S:"NOTICE"\0 + V:"NOTICE"\0 + C:"00000"\0 + M:<msg>\0 + NUL
            let body_len = 1+6+1 + 1+6+1 + 1+5+1 + 1+message.len()+1 + 1;
            buf.reserve(1 + 4 + body_len);
            buf.put_u8(b'N');
            buf.put_i32(4 + body_len as i32);
            buf.put_u8(b'S'); write_cstring(buf, "NOTICE");
            buf.put_u8(b'V'); write_cstring(buf, "NOTICE");
            buf.put_u8(b'C'); write_cstring(buf, "00000");
            buf.put_u8(b'M'); write_cstring(buf, message);
            buf.put_u8(0);
        }
...
fn read_cstring(buf: &mut BytesMut) -> Result<String, String> {
    if let Some(pos) = buf.iter().position(|&b| b == 0) {
        // split_to(pos) gives us exactly the string bytes without a separate to_vec() copy;
        // advance(1) discards the NUL terminator.
        let chunk = buf.split_to(pos);
        buf.advance(1);
