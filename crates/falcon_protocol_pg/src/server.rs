...
async fn handle_connection_with_timeout(
    ...
    loop {
        ...
        // Pipeline buffer: accumulates all extended-query responses (Parse/Bind/Describe/Execute)
        // and is flushed to the socket only on Sync or Flush messages.  This collapses the
        // typical P+B+D+E+S flow from 5-7 write syscalls down to 1 write + 1 flush.
            ...
            match msg {
                FrontendMessage::Query(sql) => {
                    ...
                    let responses = if session.statement_timeout_ms > 0 {
                        ...
                        // Use Arc<str> to avoid a heap allocation on clone — the SQL string
                        // is already owned; wrap once and share the pointer with spawn_blocking.
                        let sql_arc: std::sync::Arc<str> = sql.as_str().into();
                        let mut task = tokio::task::spawn_blocking({
                            let sql = std::sync::Arc::clone(&sql_arc);
                ...
                FrontendMessage::Parse {
                    ...
                    // ParseComplete: accumulate into pipeline_buf; flushed on Sync.
                    pipeline_buf.extend_from_slice(&[0x31, 0, 0, 0, 4]);
                }
                FrontendMessage::Bind {
                    ...
                    // BindComplete: accumulate into pipeline_buf; flushed on Sync.
                    pipeline_buf.extend_from_slice(&[0x32, 0, 0, 0, 4]);
                }
                FrontendMessage::Describe { kind, name } => {
                    ...
                    if kind == b'S' {
                        ...
                        if let Some(ps) = session.prepared_statements.get(&name) {
                            ...
                            codec::encode_message_into(&mut pipeline_buf, &BackendMessage::NoData);
                        }
                    } else {
                        // Portal describe
                ...
                FrontendMessage::Execute { portal, .. } => {
                    ...
                    if let Some(p) = portal_data {
                        ...
                        // Accumulate into pipeline_buf; flushed on Sync.
                        for response in &responses {
                ...
                FrontendMessage::Sync => {
                    // Flush notifications + ReadyForQuery together with all accumulated
                    // pipeline responses in a single write syscall.
                    for notif in session.notifications.drain_pending() {
                        codec::encode_message_into(
                            &mut pipeline_buf,
                            &BackendMessage::NotificationResponse {
                ...
                FrontendMessage::Close { kind, name } => {
                    ...
                    // CloseComplete into pipeline_buf; flushed on Sync.
                    codec::encode_message_into(
                        &mut pipeline_buf,
                ...
                FrontendMessage::Flush => {
                    // Explicit Flush: drain pipeline_buf immediately.
