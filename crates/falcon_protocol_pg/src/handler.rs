...
impl QueryHandler {
    ...
    pub(crate) fn single_row_result(
        ...
        let mut out: Vec<BackendMessage> = vec![BackendMessage::RowDescription { fields }];
        for row in rows {
            out.push(BackendMessage::DataRow { values: row });
        }
        out.push(BackendMessage::CommandComplete { tag: "SELECT".into() });
        out
    }
} // end impl QueryHandler (part 1)
...
/// Format a DML command completion tag into a heap-allocated String without
/// using the `format!` macro's Formatter machinery.
///
/// For the common single-row cases the tag string is a static literal (zero alloc).
/// For all other cases the count is written into a 20-byte stack buffer via
/// manual digit encoding, then assembled into a String in a single allocation.
pub(crate) fn fmt_dml_tag(verb: &str, count: u64) -> String {
    match (verb, count) {
        ("INSERT", 0) => return "INSERT 0 0".to_owned(),
        ("INSERT", 1) => return "INSERT 0 1".to_owned(),
        ("UPDATE", 0) => return "UPDATE 0".to_owned(),
        ("UPDATE", 1) => return "UPDATE 1".to_owned(),
        ("DELETE", 0) => return "DELETE 0".to_owned(),
        ("DELETE", 1) => return "DELETE 1".to_owned(),
        ("SELECT", 0) => return "SELECT 0".to_owned(),
        ("SELECT", 1) => return "SELECT 1".to_owned(),
        _ => {}
    }
    // Write count digits into a 20-byte stack buffer (u64::MAX is 20 digits).
    ...
    let count_str = std::str::from_utf8(&digits[..len]).unwrap_or("0");
    if verb == "INSERT" {
        format!("INSERT 0 {count_str}")
    } else {
        format!("{verb} {count_str}")
    }
}
